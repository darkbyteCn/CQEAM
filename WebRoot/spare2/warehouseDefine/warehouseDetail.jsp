    ...
    if (formValidate(fieldNames, fieldLabels, EMPTY_VALIDATE)) {
        ...
        if (codeExist == "N") {
            ...
            if (categoryExist == "N") {
                ...
                mainFrm.submit();
                opener.location.href = "/servlet/com.sino.ams.spare2.inv.servlet.EtsObjectServlet?act=<%=WebActionConstant.QUERY_ACTION%>";
            } else {
                alert("该类型仓库已存在");
            }
        } else {
...
<%--   function do_delete() {
   ...
   document.mainFrm.action = "/servlet/com.sino.ams.spare2.inv.servlet.EtsObjectServlet";
...
function checkObjectCode() {
    ...
    if (objCode != "") {
        url = "/servlet/com.sino.ams.spare2.inv.servlet.EtsObjectServlet?act=CHECK_CODE_ACTION&workorderObjectCode=" + objCode;
...
function checkObjectCategory() {
    ...
    xmlHttp = createXMLHttpRequest();
    if (objCategory != "") {
        url = "/servlet/com.sino.ams.spare2.inv.servlet.EtsObjectServlet?act=CHECK_CATEGORY_ACTION&objectCategory=" + objCategory;
