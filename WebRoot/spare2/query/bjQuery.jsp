...
<%@ page import="com.sino.ams.spare2.query.dto.AmsBjQueryDTO" %>
...
<form action="/servlet/com.sino.ams.spare2.query.servlet.AmsBjQueryServlet" name="mainForm" method="post">
<%
...
<table border="0" width="100%" class="queryHeadBg">
    ...
    <tr>
        ...
        <td><select name="company" id="company1" style="width:80%">
        ...
        <td width="10%" align="left"><input type="text" name="workorderObjectName" style="width:80%"
                ...
                href="#" class="linka" style="cursor:'hand'" onclick="do_selectName();">[…]</a></td>
    </tr>
    <tr>
        ...
        <td width=8% align="center"><img src="/images/eam_images/search.jpg" alt="查询备件"
    ...
    <tr>
        <td align="right">地市：</td>
        <td><select name="company" id="company2" style="width:80%">
            <%=request.getAttribute("OU")%>
        </select></td>
    ...
    <tr>
        ...
        <td width="15%"><select name="company" id="company3" style="width:80%">
            <%=request.getAttribute("OU")%>
        </select></td>
        ...
        <td width=8% align="center"><img src="/images/eam_images/search.jpg" alt="查询备件"
                                         onClick="do_Search(); return false;"><img src="/images/eam_images/export.jpg"
...
<%--</div>--%>
<script type="text/javascript">
    var columnArr = new Array("所属公司", "物料编码", "备件名称", "规格型号", "数量", "设备类型", "所属地点");
    var widthArr = new Array("15%", "12%", "16%", "17%", "8%", "12%", "20%");
    ...
    <table width="100%" border="1" bordercolor="#666666" id="dataTab">
        <%
            ...
            for (int i = 0; i < rows.getSize(); i++) {
                row = rows.getRow(i);
        %>
        <tr height="20" onclick="do_ShowDetail('<%=row.getValue("BARCODE")%>')"
            onMouseMove="style.backgroundColor='#EFEFEF'"
            onMouseOut="style.backgroundColor='#ffffff'">
            <td height="22" width="15%"><%=row.getValue("COMPANY")%>
            </td>
            <td height="22" width="12%"><%=row.getValue("BARCODE")%>
            </td>
            <td height="22" width="16%"><%=row.getValue("ITEM_NAME")%>
            </td>
            <td height="22" width="17%"><%=row.getValue("ITEM_SPEC")%>
            </td>
            <td height="22" width="8%" align="right"><%=row.getValue("QUANTITY")%>
            </td>
            <td height="22" width="12%"><%=row.getValue("ITEM_CATEGORY_NAME")%>
            </td>
    ...
    <table width="100%" border="1" bordercolor="#666666" id="dataTab1">
        <%
            ...
            for (int i = 0; i < rows.getSize(); i++) {
                row = rows.getRow(i);
        %>
        <tr  onclick="do_ShowDetail('<%=row.getValue("BARCODE")%>')"
            onMouseMove="style.backgroundColor='#EFEFEF'"
            onMouseOut="style.backgroundColor='#ffffff'">
            <td height="22" width="15%"><%=row.getValue("COMPANY")%>
            </td>
