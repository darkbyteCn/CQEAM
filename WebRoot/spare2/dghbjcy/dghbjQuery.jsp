...
<html>
  ...
  %>
  <form action="/servlet/com.sino.ams.spare2.dghbjcy.servlet.AmsWaitBackServlet" name="mainForm" method="post">
        <script type="text/javascript">
    printTitleBar("待归还备件查询")
</script>
    ...
    <input type="hidden" name="act" value="<%=action%>">
       ...
       <script type="text/javascript">
        var columnArr = new Array("单据号", "单据日期", "申领人", "设备名称", "型号", "申请数量", "调拨仓库", "调拨数量", "预计归还日期");  // 去掉"实际接收日期",因为一个调拨单行可能发运多次,所以有多个日期
        var widthArr = new Array("12%", "8%", "8%", "15%", "15%", "8%", "10%", "8%", "10%");
    ...
    <div style="overflow-y:scroll;height:345px;width:100%;left:1px;margin-left:0px"
        ...
        <table width="100%" border="1" bordercolor="#666666" id="dataTab">
            ...
            <tr height="22" onMouseMove="style.backgroundColor='#EFEFEF'"
                ...
                <td width="8%" align="center"><%=row.getValue("TRANS_DATE")%></td>
                <td width="8%"><%=row.getValue("USERNAME")%></td>
                <td width="15%"><%=row.getValue("ITEM_NAME")%></td>
                <td width="15%"><%=row.getValue("ITEM_SPEC")%></td>
                <td width="8%" align="right"><%=row.getValue("APPLY_QUANTITY")%></td>
                <td width="10%"><%=row.getValue("FROM_OBJECT_NAME")%></td>
                <td width="8%" align="right"><%=row.getValue("MOVE_QUANTITY")%></td>
                <%--<td height="22" width="10%"><%=row.getValue("RCV_DATE")%></td>--%>
                <td width="10%" align="center"><%=row.getValue("RESPECT_RETURN_DATE")%></td>
            </tr>
            <%
                }      }
            %>
