...
<html>
  ...
  <body text="000000" bgcolor="ffffff" leftmargin="0" topmargin="0" class="STYLE1" onload="window.focus();">
    <%
		...
		String[] storemanIds = (String[])request.getAttribute("storemanIds");
		request.setAttribute("storemanIds", storemanIds);
	%>
...
function do_submit() {
	...
	var tempStr = "/servlet/com.sino.nm.ams.inv.storeman.servlet.EamInvStoremanServlet?userId=" + document.forms[0].userId.value;
	var pStr=""; 
 	<%
 		String str = "";
 		for(int i=0; i<gets.length; i++){
    		str = str + ("&gets=" + gets[i] + "&storemanIds=" + storemanIds[i]);
    	} 	
    %>
    pStr = "<%=str%>";
