...
%>
<form method="post" name="mainFrm"  action="/servlet/com.sino.ams.dzyh.servlet.EtsObjectServlet">
...
<table width="100%" border="0" class="queryHeadBg">
        <tr>
            ...
            <td width="30%"><select style="width:80%" name="countyCode"><%=request.getAttribute(WebAttrConstant.COUNTY_OPTION)%></select></td>
        </tr>
</table>
...
<%--<div style="position:absolute;top:73px;left:0px;width:833px">--%>
    ...
    <table width="100%" class="headerTable" border="1">
        <tr height="20">
            ...
            class="headCheckbox" id="controlBox" onclick="checkAll('titleCheck','workorderObjectNos')"></td>
...
%>
        <tr height="22" style="cursor:'hand'" onMouseMove="style.backgroundColor='#EFEFEF'" onMouseOut="style.backgroundColor='#ffffff'">
            ...
            <input type="checkbox" name="workorderObjectNos" value="<%=row.getValue("WORKORDER_OBJECT_NO")%>"></td>
            
            <td width="12%" align="center" onclick="show_detail('<%=row.getValue("WORKORDER_OBJECT_NO")%>')">
            <%=row.getValue("WORKORDER_OBJECT_CODE")%></td>
            <td width="23%" onclick="show_detail('<%=row.getValue("WORKORDER_OBJECT_NO")%>')">
            <%=row.getValue("WORKORDER_OBJECT_NAME")%></td>
            <td width="30%" align="left" onclick="show_detail('<%=row.getValue("WORKORDER_OBJECT_NO")%>')">
            <%=row.getValue("WORKORDER_OBJECT_LOCATION")%></td>
            <td width="10%" align="center" onclick="show_detail('<%=row.getValue("WORKORDER_OBJECT_NO")%>')">
            <%=row.getValue("COUNTY_NAME")%></td>
            <td width="12%" onclick="show_detail('<%=row.getValue("WORKORDER_OBJECT_NO")%>')">
            <%=row.getValue("ISALL")%></td>
            <td width="10%" align="center" onclick="show_detail('<%=row.getValue("WORKORDER_OBJECT_NO")%>')">
            <%=row.getValue("DISABLE_DATE")%></td>
        </tr>
<%
	    } }
%>
...
function do_search() {
    ...
    mainFrm.action = "/servlet/com.sino.ams.dzyh.servlet.EtsObjectServlet";
    mainFrm.submit();
}

function show_detail(workorderObjectNo) {
    mainFrm.act.value = "<%=WebActionConstant.DETAIL_ACTION%>";
    mainFrm.action = "/servlet/com.sino.ams.dzyh.servlet.EtsObjectServlet?&workorderObjectNo="+workorderObjectNo +"&categoryName="+mainFrm.categoryName.value;
    mainFrm.submit();
}
...
//    alert(workorderObjectNos);
    createXMLHttpRequest();
        url = "/servlet/com.sino.ams.dzyh.servlet.EtsObjectServlet?act=verifyObjectNos&workorderObjectNos="+workorderObjectNos;
