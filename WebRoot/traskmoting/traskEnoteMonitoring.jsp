    ...
    <form name="listform" action="/servlet/com.sino.traskmoting.servlet.SfActInfoTaskServlet" method="POST">
       ...
       function printTool(){
           ...
           toolBar.SinoActions = new Array(ArrAction,ArrAction1,ArrAction4,ArrAction5);
            ...
            toolBar.treeViewTitle = new Array("被催办者","<input type=checkbox name='checkAll' STYLE='cursor:pointer' onclick='ChangeCheckListEx();'>","任务名称","关键字","主题","签收时间","预定时间","");
            toolBar.treeViewWidth = new Array("4%","2%","15%","19%","15%","20%","20%","2%");
               toolBar.print();
        }
				...
				<table   width="100%">
					<%
						...
						if (ds != null && !ds.isEmpty()) {
							SfActInfoDTO std;
       							String tridName = null, subIdName = null, btidName = null, btidSubName = null, userName = null, signStatus = null;
					...
					<tr id="<%=tridName%>">
						<td colspan="8">
							<img id='<%=btidName%>' STYLE='cursor:pointer' border="0" alt="expand" src="/images/expand.gif" onclick="doOutline(this)" />&nbsp;<font color="blue"><%=std.getSfactPickUser()%></font></td>
                    ...
                    <%
                        ...
                        String subName;
                        if(!status.equals(signStatus) || isNewStatus) {
                            isNewStatus = false;
                            l = 1;
                    ...
                    <td width="2%"></td>
                    <td colspan="7" ><img id='<%=btidSubName%>' STYLE='cursor:pointer' border="0" alt="expand" src="/images/expand.gif" onclick="doOutline(this)">&nbsp;<font color="blue"><%=std.getSfactSignStatus()%></font></td><td bordercolordark=white bordercolorlight=white nowrap></td>
                    </tr>
                    <%
                            l = 1;
                            signStatus = status;
...
//    signDueDateCal.setTime(now.getTime());
                    ...
                    <td width="15%"bordercolordark=white bordercolorlight=white nowrap align="LEFT"><font color="black"><table border=0 width=100% style='cursor:pointer'><tr><td  nowrap  onclick="do_ShowDetail('<%=std.getSfactActId()%>'); return false;"><img border="0" alt="priority" src="<%=priorityGif%>"> <%=std.getSfactTaskName()%></td></tr></table></font></td>
                    <td width="19%" bordercolordark=white bordercolorlight=white nowrap align="LEFT"><font color="black"><table border=0 width=100% style='cursor:pointer'><tr><td onclick="do_ShowDetail('<%=std.getSfactActId()%>'); return false;"><%=std.getSfactApplColumn1()%></td></tr></table></font></td>
                    <td width="15%" bordercolordark=white bordercolorlight=white nowrap align="LEFT"><font color="black"><table border=0 width=100% style='cursor:pointer'><tr><td onclick="do_ShowDetail('<%=std.getSfactActId()%>'); return false;"><%=std.getSfactApplColumn2()%></td></tr></table></font></td>
                    <%
                        if(std.getSfactSignStatus().trim().equals("在办")) {
                    %>
                    <td width="20%" bordercolordark=white bordercolorlight=white nowrap align="LEFT"><font color="black"><table border=0 width=100% style='cursor:pointer'><tr><td nowrap  onclick="do_ShowDetail('<%=std.getSfactActId()%>'); return false;"><%=std.getSfactSignDate()%></td></tr></table></font></td>
                    ...
                    %>
                    <td width="20%" bordercolordark=white bordercolorlight=white nowrap align="LEFT"><font color="black"><table border=0 width=100% style='cursor:pointer'><tr><td nowrap  onclick="do_ShowDetail('<%=std.getSfactActId()%>'); return false;">未签收</td></tr></table></font></td>
