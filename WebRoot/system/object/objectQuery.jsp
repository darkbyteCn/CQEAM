...
%>
<form method="post" name="mainFrm" action="/servlet/com.sino.ams.system.object.servlet.CommonObjectServlet">

    <input type="hidden" name="checkedObjectNo" id="checkedObjectNo" value="">
    <script type="text/javascript">
        printTitleBar("地点统一维护");
        ...
        var ArrAction7 = new Array(false, "传送", "copy.gif", "传送", "do_Transfer()");
        var ArrActions = new Array(ArrAction0, ArrAction1, ArrAction2, ArrAction3, ArrAction4, ArrAction5, ArrAction6, ArrAction7);
    ...
    <table width="100%" border="0" bgcolor="#FFFFFF" id="table1">
        <tr>
            ...
            <td width="17%"><select class="select_style1" style="width:100%" name="organizationId" <%if (isNew) {%>
                                    onchange="do_ChangeCounty(this);" <%}%>><%=dto.getOrganizationOption()%>
            </select></td>
            <td width="8%" align="right">创建日期：</td>
            <td width="17%"><input style="width:100%" type="text" class="input_style1" name="startDate"
                                   value="<%=dto.getStartDate()%>" title="点击选择起始日期" readonly
                                   onclick="gfPop.fStartPop(startDate, endDate)"></td>
            <td width="8%" align="right">到：</td>
            <td width="17%"><input style="width:100%" type="text" class="input_style1" name="endDate"
                                   value="<%=dto.getEndDate()%>" title="点击选择截至日期" readonly
                                   onclick="gfPop.fEndPop(startDate, endDate)"></td>
            <td width="9%" align="right">行政区划空：</td>
            <td width="16%"><input type="checkbox" name="areaTypeIsNull"
                                   value="true" <%=dto.isAreaTypeIsNull() == true ? "checked" : " "%>></td>
        </tr>
        <tr>
            <td width="8%" align="right">所属区域：</td>
            <td width="17%"><select class="select_style1" style="width:100%"
                                    name="countyCode"><%=dto.getCountyOption()%>
            </select></td>
            <td width="8%" align="right">行政区划：</td>
            <td width="17%"><select class="select_style1" style="width:100%"
                                    name="areaType"><%=dto.getAreaTypeOption() %>
            </select></td>
            <td width="8%" align="right">地点专业：</td>
            <td width="17%">
                <select class="select_style1" style="width:100%" name="objectCategory"><%=dto.getObjCategoryOption()%>
                </select>
            </td>
        ...
        <tr>
            ...
            <td width="17%"><input class="input_style1" style="width:100%" type="text" name="workorderObjectCode"
                                   value="<%=dto.getWorkorderObjectCode()%>"></td>
            <td width="8%" align="right">地点名称：</td>
            <td width="17%"><input class="input_style1" style="width:100%" type="text" name="workorderObjectName"
                                   value="<%=dto.getWorkorderObjectName()%>"></td>
            <td width="8%" align="right">有效状态：</td>
            <td width="17%"><select class="select_style1" style="width:100%" name="enabled" id="enabled"
                                    onchange="do_SetControlBtnEnable();"><%=dto.getEnableOption()%>
            </select></td>
            <td width="9%" align="right"></td>
            <td width="16%"></td>
        </tr>
    ...
    <div id="headDiv" style="overflow:hidden;position:absolute;top:123px;left:1px;width:150%">
        <table class="headerTable" border="1" style="width:200%">
            <tr height="23px" onClick="executeClick(this)" style="cursor:pointer">
                <td width="2%" align="center"><input type="checkbox" name="titleCheck" class="headCheckbox"
                                                     id="controlBox" onclick="checkAll('titleCheck','subCheck')"></td>
    ...
    <div id="dataDiv" style="overflow:scroll;height:66%;width:150%;position:absolute;top:115px;left:1px" align="left"
        ...
        <table id="dataTable" width="200%" border="1" style="TABLE-LAYOUT:fixed;word-break:break-all">
            ...
            <tr height="22" style="cursor:pointer" onMouseMove="style.backgroundColor='#EFEFEF'"
                ...
                <td style="word-wrap:break-word" title="该地点下有资产,不能失效！" height="22" width="2%" align="center" class="">
                    <input disabled="true" type="checkBox" name="workorderObjectNo"
                           value="<%=row.getValue("WORKORDER_OBJECT_NO")%>">
                </td>
                <%
                    }
                %>
                <td width="7%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly
                                                                             value="<%=row.getValue("COMPANY")%>"></td>
                <td width="10%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput2" readonly
                                                                              value="<%=row.getValue("WORKORDER_OBJECT_CODE")%>">
                </td>
                <td width="10%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly
                                                                              value="<%=row.getValue("WORKORDER_OBJECT_NAME")%>">
                </td>

                <td width="6%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly
                                                                             value="<%=row.getValue("COUNTY_NAME")%>">
                </td>
                <td width="6%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly
                                                                             value="<%=row.getValue("OBJECT_CATEGORY_NAME")%>">
                </td>
                <%--<td width="5%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly value="<%=row.getValue("IS_TD")%>"></td>--%>
                <td width="10%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly
                                                                              value="<%=row.getValue("BTS_NO")%>"></td>
                <%--<td width="6%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly value="<%=row.getValue("CREATION_USER")%>"></td>--%>

                <td width="6%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput2" readonly
                                                                             value="<%=row.getValue("CREATION_DATE")%>">
                </td>
                <td width="6%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput2" readonly
                                                                             value="<%=row.getValue("DISABLE_DATE")%>">
                </td>
                <%--<td width="6%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly value="<%=row.getValue("UPDATED_USER")%>"></td>--%>
                <td width="6%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput2" readonly
                                                                             value="<%=row.getValue("LAST_UPDATE_DATE")%>">
                </td>
                <td width="6%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly
                                                                             value="<%=row.getValue("VALUE")%>"></td>
                <td width="8%" onclick="show_Detail('<%=objectNo%>')"><input type="text" class="finput" readonly
                                                                             value="<%=row.getValue("LATITUDE_LONGITUDE")%>">
                </td>
            </tr>
            <%
                    }
...
<iframe width=174 height=179 name="gToday:normal:calendar.js" id="gToday:normal:calendar.js"
...
function do_SetControlBtnEnable() {
    ...
    if (enabled == "Y" || enabled == "") {
        ...
        ShowSinoButton(transferPic);
    } else if (enabled == "N") {
        HideSinoButton(disPic);
        ShowSinoButton(enPic);
        HideSinoButton(transferPic);
    }
    if ("<%=provinceCode%>" == "<%=DictConstant.PROVINCE_CODE_JIN%>") {
        HideSinoButton(3);
        HideSinoButton(6);
        HideSinoButton(7);
    }
...
function do_SYN() {             //同步地点
    ...
    if (confirm("确认同步吗？继续请点“确定”按钮，否则请点“取消”按钮。")) {
        ...
        mainFrm.action = "/servlet/com.sino.ams.system.object.servlet.CommonObjectServlet";
        mainFrm.submit();
    }
}
