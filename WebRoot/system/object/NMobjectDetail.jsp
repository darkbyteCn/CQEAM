			...
			<table width="90%" align="center" border="1" bordercolor="#666666">
				<tr>
					...
					<td width="80%" colspan="3">
						<select class='select_style1' name="organizationId" style="width: 100%" onchange="do_ChangeOrg(this);"><%=dto.getOrganizationOption()%></select>
					</td>
				</tr>
				<tr>
					<td width="20%" align="right">
					...
					<td width="80%" colspan="3">
						<select class='select_style1' name="objectCategory" style="width: 100%" onchange="to_object_code()" onclick="do_Cancle();"><%=dto.getObjCategoryOption()%></select>
					</td>
				</tr>
				<tr>
					...
					<td width="35%">
						<select name="countyCode" class="select_style1" style="width: 100%" onchange="do_ProduceWorkOrderObjectCode()"><%=dto.getCountyOption()%></select>
					</td>
					<td align="right" width="15%">
						经纬度：
					</td>
				...
				<tr>
					...
					<td colspan="3">
						<select onclick="do_Cancle();" name="areaType" class="select_style1" style="width: 100%" disabled="true"><%=dto.getAreaTypeOption()%></select>
					</td>
				</tr>
				<tr>
					<td align="right">
					...
					<td colspan="3">
						<select class="select_style1" id="city" name="city" onchange="do_ChangeCounty(this);" style="width: 100%" disabled="true"><%=dto.getCityOption()%></select>
					</td>
				</tr>
				<tr>
					<td align="right">
					...
					<td colspan="3">
						<select class="select_style1" id="county" name="county" style="width: 100%" disabled="true"><%=dto.getCountyOptions()%></select>
					</td>
				</tr>
<%
...
%>
				...
				<tr>
					...
					<td colspan="3">
						<input class='input_style1' name="workorderObjectName" type="text" id="workorderObjectName" value="<%=dto.getWorkorderObjectName()%>" style="width: 100%" readonly>
					</td>
				</tr>
<%
    } else {
%>
				...
				<tr>
					...
					<td colspan="3">
						<input name="workorderObjectName" type="text" id="workorderObjectName" value="<%=dto.getWorkorderObjectName()%>" class='input_style1' readonly style="width: 100%">
					</td>
				</tr>
<%
    }
%>
				...
				<tr>
					...
					<td colspan="3">
						<textarea name="objetcRemark" height="120" type="areatext" id="objetcRemark" value="" style="width: 100%" readonly></textarea>
					</td>
				</tr>
				<tr>
					...
					<td height="60" colspan="3">
						<textarea name="remark" onclick="do_Cancle();" id="remark" value="<%=StrUtil.htmlStrEncode(dto.getRemark())%>" class="input_style1" style="width: 100%; height: 100%" readonly><%=dto.getRemark()%></textarea>
					</td>
				</tr>
			</table>
...
function do_SaveObject() {
    ...
    var fieldLabels = "公司名称;地点专业;所属区域;地点编号;市;区/县;描述;地点名称;是否TD地点";
    ...
    if (isHasObject=="N") {
    	...
    	fieldLabels = "公司名称;地点专业;所属区域;地点编号;描述;地点名称;是否TD地点";
    }    
    if (formValidate(fieldNames, fieldLabels, EMPTY_VALIDATE)){
        ...
        if (mainFrm.warnWorkorderObjectName.value != "") {
        	return false;
        }
        if(objLength==25){
            mainFrm.act.value = "<%=WebActionConstant.SAVE_ACTION%>";
            mainFrm.city.disabled = false;
            mainFrm.isTd.disabled = false;
            do_ControlEditable(false);
            mainFrm.submit();
            self.close();
            opener.do_Search();
        } else {
            alert("请确认地点编号已经填写正确，必须是公司代码加地点类型加8位流水号一共25位！")
        }
    }
}
...
function do_ResponseObjectCode(resText){
    ...
    with(document){
        if(resText == "Y"){
        	if (objectCat=="10") {
        		getElementById("warnExist").innerText = "对不起，该基站编号已存在!";
        	} else {
        		getElementById("warnExist").innerText = "对不起，该营业厅编号已存在!";
        	}
			
            getElementById("warnExist").style.visibility = "visible";
            getElementById("savePic").style.visibility = "hidden";
            if(getElementById("newPic")){
                getElementById("newPic").style.visibility = "hidden";
            }
        } else {
        	getElementById("warnExist").innerText = "";
            getElementById("warnExist").style.visibility = "hidden";
            getElementById("savePic").style.visibility = "visible";
            if(getElementById("newPic")){
                getElementById("newPic").style.visibility = "visible";
            }
        }
    }
}
...
function do_InitBts(objectCat) {
	if(objectCat=="10"){   
    	document.getElementById("btsTd").innerText = "基站编号："; 	
    	document.getElementById("bts").style.display="block";
    } else if (objectCat=="50") {
    	document.getElementById("btsTd").innerText = "营业厅编号：";
    	document.getElementById("bts").style.display="block";
    } else {
...
function do_initPage(){
    window.focus();
    var act = document.getElementById("act").avlue;
    if(act == ""){
