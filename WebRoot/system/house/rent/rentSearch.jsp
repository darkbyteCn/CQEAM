...
<script type="text/javascript">
    printTitleBar("租赁房屋土地查询")
</script>

<table border="0" width="100%" class="queryTable">
    <tr>
        <td width="10%" align="right">条码：</td>
        <td width="14%" align="left"><input type="text" name="barcode" class="input_style1" style="width:100%"
        ...
        <td width="14%" align="left"><input name="itemSpec" class="input_style1" style="width:100%" value = "<%=housedto.getItemSpec()%>"></td>
    ...
    <tr <%--id="certificate" style="display:none;"--%>>
        ...
        <td align="left"><input type="text" name="fromDate" value="<%=fromDate%>" readOnly  class="input_style2"
                                    ...
                                    <!--<img src="/images/calendar.gif" alt="点击选择日期" onclick="gfPop.fStartPop(fromDate,toDate);">-->
        </td>
        <td align="right"> 到：</td>
        <td align="left"><input type="text" name="toDate" value="<%=toDate%>" readOnly style="width:100%"  class="input_style2"
        ...
        <td align="right"><%--<img  src="/images/eam_images/new_add.jpg" alt="新增信息" onClick=" do_creat(); return false;">--%></td>
        <td align = "left"><img src="/images/eam_images/search.jpg"  alt="查询未处理信息" onClick="do_search(); return false;"><img src="/images/eam_images/export.jpg" id="queryImg" style="cursor:'hand'" onclick="do_Export();" alt="导出到Excel"></td>
    </tr>
</table>
<input type="hidden" name="act" value="<%=action%>">
<div id="headDiv" style="overflow:hidden;position:absolute;top:95px;left:1px;width:1180px">
    <table class="headerTable" border="1" width="150%" >
        <tr>
            ...
            <td height="22" width="15%" align="center">地点</td>
            <td height="22" width="6%" align="center">是否基站</td>
            <td height="22" width="8%" align="center">类型</td>
            ...
            <td height="22" width="7%" align="center">出租单位</td>
	...
	%>
	<div id="dataDiv" style="overflow:scroll;height:70%;width:1197px;position:absolute;top:119px;left:1px" align="left" onscroll="document.getElementById('headDiv').scrollLeft = this.scrollLeft;">
        ...
        <%
            ...
            for (int i = 0; i < rows.getSize(); i++) {
                row = rows.getRow(i);
        %>
        <!-- <tr class="dataTR" onclick="show_detail2('<%=row.getValue("BARCODE")%>','<%=row.getValue("SYSTEMID")%>','<%=row.getValue("OFFICE_USAGE")%>','<%=row.getValue("OFFICE_TYPE")%>','<%=row.getValue("RENT_ID")%>')"> -->
        <tr class="dataTR">    
            <td height="22" width="7%" align="center"><%=row.getValue("BARCODE")%>
            </td>
            <td height="22" width="14%" align="left"><%=row.getValue("ITEM_NAME")%>
            </td>
            <td height="22" width="15%" align="left"><%=row.getValue("ITEM_SPEC")%>
            </td>
            <td height="22" width="15%" align="left"><%=row.getValue("HOUSE_ADDRESS")%>
            </td>
            <td height="22" width="6%" align="left"><%=row.getValue("OFFICE_USAGE")%>
            </td>
            <td height="22" width="8%" align="left"><%=row.getValue("OFFICE_TYPE")%>
            </td>
            ...
            <td height="22" width="7%" align="center"><%=row.getValue("RENT_UNIT")%>
            </td>
...
function do_search() {
    ...
    mainFrm.action = "/servlet/com.sino.ams.system.house.servlet.RentHouseSearchServlet";
    mainFrm.submit();
}

function do_creat() {
       var url = "/servlet/com.sino.ams.system.house.servlet.RentHouseSearchServlet?act=CREATE_ACTION";
...
function show_detail(barcode, systemId, officeUsage, officeType,rentId) {
    var height = "400px";
    if (((officeUsage == "非基站") && (officeType == "房屋")) || ((officeUsage == "非基站") && (officeType == "房地合一"))) {
        height = "450px";
    }
     var url = "/servlet/com.sino.ams.system.house.servlet.RentHouseSearchServlet?act=DETAIL_ACTION&barcode=" + barcode +"&bts=" + officeUsage + "&category=" + officeType +"&rentId="+rentId +"&look=1";
...
function show_detail2(barcode, systemId, officeUsage, officeType,rentId) {
     var url = "/servlet/com.sino.ams.system.house.servlet.RentHouseSearchServlet?act=CHECK_ACTION&barcode=" + barcode +"&bts=" + officeUsage + "&category=" + officeType +"&rentId="+rentId +"&look=1";
    var winStyle = "height=450,width=1010px,top=150, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=yes,help=no";
    window.open(url, "_blank", winStyle);
//                window.close();
}        
