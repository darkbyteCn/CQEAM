...
<script language="javascript">
	...
	var ArrAction1 = new Array(true, "导出现有设备(<%=itemCategoryName%>)", "toexcel.gif", "导出现有设备(<%=itemCategoryName%>)", "do_Export('<%=DictConstant.EXPORT_LOC_ITEM%>')");
	var ArrAction2 = new Array(true, "导出已盘点设备(<%=itemCategoryName%>)", "toexcel.gif", "导出已盘点设备(<%=itemCategoryName%>)", "do_Export('<%=DictConstant.EXPORT_SCAN_ITEM%>')");
...
<script language="javascript">
	...
	tabBox.addtab("ownItems", "本专业现有设备");
	tabBox.addtab("scanItems", "已盘点设备");
//	tabBox.addtab("diffItems", "盘点差异");
	tabBox.addtab("company", "公司信息");
	printTitleBar("<%=company.getCompany()%>盘点信息(<%=dto.getItemCategoryName()%>)");
	printToolBar();
	tabBox.init();
</script>
