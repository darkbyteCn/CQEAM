...
<form action="/servlet/com.sino.ams.newasset.servlet.ItemSpareServlet" method="post" name="mainFrm">
...
<table border="1" style="width:100%;TABLE-LAYOUT:fixed;word-break:break-all">
    <tr height="20px">
        ...
        <td width="25%"><select style="width:100%" name="organizationId" class="select_style1" onchange="getDeptOption();"><%=request.getAttribute(AssetsWebAttributes.CITY_OPTION)%></select></td>
...
<div id="aa" style="overflow-x:hidden;overflow-y:scroll;position:absolute;top:101px;left:0px;width:100%" class="crystalScroll">
	<table border=1 width="270%" class="headerTable">
		<tr class=headerTable height="15px" onclick="executeClick(this)" style="cursor:hand" title="点击全选或全不选">
            ...
            <td align=center width=3%>入库日期</td>
...
<div id="dataDiv" style="overflow:scroll;height:71%;width:100%;position:absolute;top:125px;left:0px" align="left" onscroll="document.getElementById('aa').scrollLeft = this.scrollLeft;">
...
%>
        <tr class="dataTR">
          ...
          <td width=2%><input type="text" class="finput3"  readonly value="<%=row.getValue("IS_ABATE")%>"></td>
          <td width=2%><input type="text" class="finput3"  readonly value="<%=row.getValue("ITEM_UNIT")%>"></td>
...
<%
    if (hasData) {
%>
<div id="navigatorDiv" style="position:absolute;top:575px;left:0;"><%=request.getAttribute(QueryConstant.SPLIT_PAGE_HTML)%></div>
<%
    }
%>
</body>
...
function do_SelectContent() {
 ...
 var xmlHttp;
    ...
    function getDeptOption() {
        ...
        if (organizationId != "") {
            url = "/servlet/com.sino.ams.newasset.servlet.ItemSpareServlet?act=GET_DEPT_OPTION&organizationId=" + organizationId;
