...
<div id="searchDiv" style="position:absolute;top:29px;left:1px;width:100%">
    <table border="0" width="100%" style="border-collapse: collapse" id="table1">
        <tr style="height:23px">
            <td align=right width="7%" height="18">任务编号：</td>
        ...
        <tr>
            ...
            <td align=right width="7%" height="40" valign="top">任务描述：</td>
...
<img src="/images/eam_images/detail_info.jpg" id="img6" alt="资产明细" onClick="do_ViewAssetsData()">
    ...
    <table class=headerTable border=1 id="headTb" style="width:120%;TABLE-LAYOUT:fixed;word-break:break-all">
        <tr height=23px style="cursor:pointer" title="点击全选或取消全选">
            ...
            <td align=center width="8%">执行周期</td>
...
%>
            <tr class="dataTR" style="display:none">
                ...
                <td width="8%"><input type="text" name="startTime" id="startTime0" class="finputNoEmpty" style="cursor:pointer;text-align:center" readonly value="" onclick="gfPop.fEndPop(referToday0, startTime0)" title="点击选择或更改该盘点工单“开始日期”"></td>
                <td width="8%"><input type="text" name="implementDays" id="implementDays0" class="finputNoEmpty" value="" onkeydown="intOnlyOnkeyDown(this.value);" onBlur="do_SetLineDays(this)"></td>
                <td width="8%"><input type="text" name="implementUser" id="implementUser0" class="finputNoEmpty" style="cursor:pointer;text-align:left" readonly value="" onclick="do_SelectUser(1, this);" title="点击选择或更改本盘点工单“执行人”"></td>
                <td width="8%"><input type="text" name="archivedUser" id="archivedUser0" class="finput" style="cursor:pointer" readonly value="" onclick="do_SelectUser(2, this);" title="点击选择或更改本盘点工单“归档人”"></td>
 ...
 function do_AppValidate(postData) {
	...
	if(isValid){
		...
		if(!postData){
			...
			<%--if(isValid){--%>
				<%--var batchId = mainFrm.batchId.value;--%>
				<%--if(batchId != "" && (<%=validOrderCount%> == 0)){--%>
					<%--alert("合法工单数为0，不能提交");--%>
					<%--isValid = false;--%>
				<%--}--%>
			<%--}--%>
