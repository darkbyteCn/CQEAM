...
<%
    ...
    String provinceOrgId = headerDTO.getServletConfig().getProvinceOrgId();
...
<body leftmargin="0" topmargin="0" onload="initPage();">
	<form action="<%=AssetsURLList.SPECIAL_ASSETS_SERVLET%>" method="post"name="mainFrm">
		...
		<jsp:include page="/newasset/specialAssetsHeader.jsp" flush="true" />
		...
		<%
			if (transType.equals(AssetsDictConstant.SPEC_ASS_RED)) {
		%>
		<input type="hidden" name="procdureName" value="特殊资产审批流程">
		<%
			} else {
		%>
	...
	%>
		统一设置：
		<input type="checkbox" name="allDept" id="allDept" checked style="display:none">
		<label for="allDept" style="display:none">新责任部门</label>
		<input type="checkbox" name="allLocation" id="allLocation">
		<label for="allLocation">调入地点</label>
		<input type="checkbox" name="allUser" id="allUser">
		<label for="allUser">新责任人</label>
		<input type="checkbox" name="allTransDate" id="allTransDate">
		<label for="allTransDate">调拨日期</label>
	<%
		}
	%>
		</legend>
		<jsp:include page="/newasset/specialAssetsLine.jsp" flush="true" />
	</fieldset>

<div
	style="position:absolute;bottom:0px;top:655px;left:0px;right:0px;width:100%;height:100%">
	<jsp:include page="/newasset/specialAssetsInclude.jsp" flush="true" />
			</div>
...
function do_SelectAssets() {
	var dialogWidth = 52;
	var dialogHeight = 29;
	var lookUpName = "<%=AssetsLookUpConstant.LOOK_SPECIAL_ASSETS%>";
	var userPara = "transType=" + mainFrm.transType.value;

    if(transferType != ""){
		userPara += "&transferType=" + transferType;
	}
	if(mainFrm.fromDept.value == ""){
		alert("请先选择部门，再选择资产！");
		mainFrm.fromDept.focus();
		return;
	}
	userPara += "&deptCode=" + mainFrm.fromDept.value;
	if(mainFrm.assetsType.value == ""){
		alert("请先选择资产类型，再选择资产！");
		mainFrm.assetsType.focus();
		return;
	}
	userPara += "&assetsType=" + mainFrm.assetsType.value;
...
function do_SelectDept(lineBox) {
	...
	var transferType = "<%=AssetsDictConstant.SPEC_ASS_RED%>";
