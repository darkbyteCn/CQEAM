...
<%
    ...
    if (transType.equals(AssetsDictConstant.ASS_RED)) {//调拨单据查询
		if(srcPage.equals(AssetsActionConstant.CANCEL_ACTION)){
			if(provinceCode.equals(AssetsDictConstant.PROVINCE_CODE_NM)){
				...
				if(transferType.equals(AssetsDictConstant.TRANS_INN_DEPT)){
					pageTitle = "部门内调拨>>撤销调拨申请";
				} else if(transferType.equals(AssetsDictConstant.TRANS_BTW_DEPT)){
					pageTitle = "部门间调拨>>撤销调拨申请";
				} else if(transferType.equals(AssetsDictConstant.TRANS_BTW_COMP)){
					pageTitle = "盟市间调拨>>撤销调拨申请";
				}
			} else {
				pageTitle = "资产调拨管理>>撤销调拨申请";
			}
		} else {
			if(provinceCode.equals(AssetsDictConstant.PROVINCE_CODE_NM)){
				if(transferType.equals(AssetsDictConstant.TRANS_INN_DEPT)){
					pageTitle = "部门内调拨>>创建调拨申请";
				} else if(transferType.equals(AssetsDictConstant.TRANS_BTW_DEPT)){
					pageTitle = "部门间调拨>>创建调拨申请";
				} else if(transferType.equals(AssetsDictConstant.TRANS_BTW_COMP)){
					pageTitle = "盟市间调拨>>创建调拨申请";
				}
			} else {    			
		...
		if(srcPage.equals(AssetsActionConstant.CANCEL_ACTION)){
			pageTitle = "资产处置管理>>撤销处置申请";
		} else {
			pageTitle = "资产处置管理>>创建处置申请";
		}
		orderNoName = "处置单号";
		deptNameDesc = "申请部门";
...
%>
            ...
            <td width="12%"><select class="select_style1" name="transferType" style="width:100%" <%=disapleProp%>><%=dto.getTransferTypeOption()%></select></td>
            ...
            <td width="23%">
				<input class="input_style2" type="text" name="startDate" value="<%=dto.getStartDate()%>" style="width: 35%" title="点击选择日期" readonly onclick="gfPop.fStartPop(startDate, endDate)">
				到：<input class="input_style2" type="text" name="endDate" value="<%=dto.getEndDate()%>" style="width: 35%" title="点击选择日期" readonly onclick="gfPop.fEndPop(startDate, endDate)">
            </td>
...
<div id="transferDiv" style="position:absolute;bottom:0px;top:0px;left:0px;right:0px;z-index:10;visibility:hidden;width:100%;height:100%">
	<table width="100%" height="120">
		...
		<tr>
			<td height="60%" width="40%"></td>
			<td bgcolor="#2984CB" width="20%"><select style="width:100%" class="select_style1" name="faContentCode"><%=dto.getFaContentOption()%></select></td>
			<td height="60%" width="40%"></td>
		</tr>
	</table>
</div>
...
%>
<div id="aa" style="overflow-y:scroll;overflow-x:hidden;position:absolute;top:50px;left:0px;width:100%" class="crystalScroll">
...
<div style="overflow:scroll;height:68%;width:100%;position:absolute;top:71px;left:0px" align="left" onscroll="document.getElementById('aa').scrollLeft = this.scrollLeft;">
