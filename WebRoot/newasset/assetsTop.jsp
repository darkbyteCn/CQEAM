...
<table border="0" width="100%" height="42" bordercolor="#000000" id="table1" cellspacing="1">
	<tr>
		<td height="21" align="center"><span style="letter-spacing: 3pt"><font face="隶书" size="5" color="#FFFFFF">资产台账管理平台</font></span></td>
	</tr>
	<tr>
		<td height="21" align="center"><%=request.getAttribute(AssetsWebAttributes.ASSETS_RADIO)%>&nbsp;<a href="" onClick="do_Close(); return false;">退出资产台账</a></td>
	</tr>
</table>
...
function do_ChangeAssetsTree(obj){
	...
	if(treeCategory != "<%=AssetsWebAttributes.ASSETS_TREE_COMM_QUERY%>"){
		...
		do_TreeURL(url);
		do_AssetsURL(treeCategory);
	} else {
		parent.location.href = "<%=AssetsURLList.ASSETS_FRM_SERVLET%>?treeCategory=" + treeCategory;;
	}
}

function do_TreeURL(objectURL){
	parent.contents.location.href = objectURL;
	parent.contents.document.getElementById("$$$waitTipMsg$$$").style.visibility = "visible";
}

function do_AssetsURL(treeCategory){
	...
	assetsLocation += "?act=&treeCategory=" + treeCategory;
	parent.assetsMain.location.href = assetsLocation;
}
