...
%>
<form action="<%=AssetsURLList.ASSETS_CLEAR_SERVLET%>" method="post" name="mainFrm">
<script type="text/javascript">
    printTitleBar("资产处置管理>>创建处置申请");
</script>
    <input type="hidden" name="act" value="">
    <table width=100% bgcolor="#E9EAE9">
        <tr>
            <td width="10%" align="right">处置单号：</td>
...
<script type="text/javascript">
    var columnArr = new Array("checkbox","处置单号","单据状态", "申请公司","申请部门", "申请人", "申请日期");
...
function do_Create() {
    var url = "<%=AssetsURLList.ASSETS_CLEAR_SERVLET%>?act=<%=AssetsActionConstant.NEW_ACTION%>";
    var style = "width=1024,height=700,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no";
    window.open(url, 'discardWin', style);
}

function showDetail(transId){
    var url = "<%=AssetsURLList.ASSETS_CLEAR_SERVLET%>?act=<%=AssetsActionConstant.DETAIL_ACTION%>&transId="+transId;
    var style = "width=1024,height=700,top=0,left=0,toolbar=no,menubar=no,scrollbars=yes,resizable=yes,location=no,status=no";
    window.open(url, 'discardWin', style);
}
