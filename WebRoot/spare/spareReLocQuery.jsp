    ...
    //       OutputStream os = response.getOutputStream();
           ...
           if (rows != null && rows.getSize() > 0) {
               ...
               for (int i = 0; i < rows.getSize(); i++) {
                   row = rows.getRow(i);
                   String value;
                   dataset.addValue(Integer.parseInt(row.getStrValue("REP_NUM")), row.getStrValue("VENDOR_NAME")+"-"+row.getStrValue("SPARE_USAGE"), row.getStrValue("COMPANY_NAME"));
               }
           }
...
function do_Export(){                  //导出execl
    mainFrm.act.value = "<%=AMSActionConstant.LOC_EXPORT_ACTION%>";
    mainFrm.action = "/servlet/com.sino.ams.spare.servlet.SpareReRatServlet";
    mainFrm.submit();
}
