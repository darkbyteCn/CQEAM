...
%>
<form name="mainForm" action="/servlet/com.sino.ams.spare.servlet.BjhjghServlet" method="post">
<input type="hidden" name="act" value="">
<input type="hidden" name="transId" value="<%=dto.getTransId()%>">
<input type="hidden" name="transType" value="<%=DictConstant.HJGH%>">
        ...
        <td>
            <table width="100%" id="table2" cellspacing="1">
                <tr height="22">
                    ...
                    <%} else {%>
                    <td width="25%"><input type="text" name="fromObjectName" value="<%=dto.getFromObjectName()%>" style="width:85%" class="blueborderYellow"><a href="#" onClick="do_SelectObject();" title="点击选择仓库" class="linka">[…]</a></td>
                    <%}%>
                    <td width="9%" align="right">目的仓库：</td>
                    <td width="20%"><input type="text" name="toObjectName" value="省公司待修库" readonly style="width:100%" class="blueborderGray"></td>
                </tr>
                <tr height="22">
...
function do_SelectObject() {
    ...
    var projects = getLookUpValues("<%=LookUpConstant.LOOK_UP_HJGH_OBJECT%>", 48, 30);
    if (projects) {
...
function do_print() {
    ...
    var url="/servlet/com.sino.ams.spare.servlet.BjhjghServlet?act=print&transId="+headerId;
...
function validateKYL() {
    ...
    for (var i = 0; i < lineCount; i++) {
        ...
        var quantity = document.getElementById("quantity" + cc).value;
        checkKYL(barcode, fromObjectNo, quantity);
        if (document.mainForm.errorKyl.value == "Y") {
            alert("由于并发因素，数量大于可用量，页面将刷新请重新填写！");
            return false;
        }
    }
    return true;
...
function checkKYL(barcode, fromObjectNo, quantity) {
    ...
    if (barcode) {
        url = "/servlet/com.sino.ams.spare.servlet.BjhjghServlet?act=CHECK_KYL&barcode=" + barcode + "&fromObjectNo=" + fromObjectNo + "&qtyObj=" +quantity;
