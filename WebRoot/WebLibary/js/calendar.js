...
function do_CustomizeCalendar(){
    if(eval(parent.getCalendarPostProcessor)){
        var postProcessor = parent.getCalendarPostProcessor();
        if(postProcessor != undefined && postProcessor != null){
            var postProcessorName = postProcessor.postProcessorName;
            if(postProcessorName != undefined && postProcessorName != null && postProcessorName != ""){
                postProcessorName = "parent." + postProcessorName;
                var processor = eval(postProcessorName);
                var argumentType = postProcessor.argumentType;
                if(!argumentType){
                    argumentType = 1;
                }
                if(argumentType == 1){
                    processor.apply(this, new Array(gdCtrl));
                } else {
                    processor.apply(this, new Array(gdCtrl.value));
                }
            }
        }
    }
