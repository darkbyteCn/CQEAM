...
<%
	if(rows != null && !rows.isEmpty()){
		for(int i = 0; i < rows.getSize(); i++){
			row = rows.getRow(i);
%>					
			<tr style="cursor:hand" onclick="do_ShowScanInfo('<%=row.getValue("WORKORDER_OBJECT_NO")%>')" title="点击查看地点“<%=row.getValue("WORKORDER_OBJECT_NAME")%>”巡检情况" onMouseMove="style.backgroundColor='#EFEFEF'" onMouseOut="style.backgroundColor='#ffffff'">
...
function do_ShowScanInfo(objectNo){
	parent.do_ShowScanInfo(objectNo);
}
</script>
