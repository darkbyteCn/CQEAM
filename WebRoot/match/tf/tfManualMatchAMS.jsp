...
<form action="/servlet/com.sino.ams.match.servlet.TFManualMatchAMS" name="mainForm" method="post">
    ...
    <table border="0" width="100%" class="queryTable">
        ...
        <tr>
            ...
            <td width="20%" colspan="4"><input class="input_style1" type="text" readonly name="workorderObjectName" value="<%=dto.getWorkorderObjectName()%>"
                                   style="width:80%" ><a href="#" onClick="do_selectObject();"
        ...
        <table class="eamHeaderTable" border="1" width="350%" >
            <tr height="22">
                ...
                <td align="center" width="2%">
                	<input type="checkbox" name="titleCheck" class="headCheckbox" onclick="checkAll(this.name,'subCheck');">
                </td>
                 -->
    ...
    <div style="overflow:scroll;position:absolute;top:118px;left:0px;height:72%;width:100%"
		...
		<table width="350%" border="1" bordercolor="#666666" id="dataTable"
            ...
            <tr onclick="executeClick(this);" onMouseMove="style.backgroundColor='#EFEFEF'"
                ...
                <!-- 
                <td height="22" width="2%" align="center">
                	<input type="checkbox" name="subCheck" value="<%//=row.getValue("SYSTEMID")%>">
                </td>
                 -->
                <td height="22" width="2%" align="center">
                	<input type="radio" name="systemId" value="<%=row.getValue("SYSTEMID")%>">
                </td>
...
function do_query() {
       if(document.getElementById("workorderObjectName").value==""){
          alert("请选择你需要匹配的地点！")         ;
           return;
       }
...
function do_selectObject() {
    ...
    var projects = getLookUpValues("LOOK_UP_TF_ASSETS_OBJECT", 48, 30, "organizationId=<%=user.getOrganizationId()%>&isCheck="+isCheck);
    //var projects = getLookUpValues("LOOK_UP_TF_ASSETS_OBJECT", 48, 30, "organizationId=<%=user.getOrganizationId()%>");
    if (projects) {
...
function matchIt() {
    ...
    //    return false;
    //}
    var amsRadioObj = mainForm.systemId;
    //alert(amsRadioObj.length);
    //return false;
    if (amsRadioObj == null || amsRadioObj == '')
    {
        alert('请选择资产后再操作！');
        return false;
    }
    var amsRadioCode;
    if (amsRadioObj.length) {
        for (var i = 0; i < amsRadioObj.length; i++) {
            if (amsRadioObj[i].checked) {
                amsRadioCode = amsRadioObj[i].value;
                break;
            }
        }
    } else {
        if (amsRadioObj.checked) {
            amsRadioCode = amsRadioObj.value;
        }
    }
    if (!amsRadioCode) {
...
function matchByCondition(condition) {
    ...
    var systemId = getRadioValue("systemId");  
...
function do_verify() {
    ...
    url = "/servlet/com.sino.ams.match.servlet.TFManualMatchAMS?act=VERIFY&systemId=" + systemId + "&assetId=" + assetId;
