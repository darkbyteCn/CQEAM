 ...
 //                   procName + "\"";
    ...
    <script language="javascript">
        function openViewWindow(pjName, pcName) {
            ...
            window.open(vplink,"", f1);
        }
        function openCaseWindow(pjName, pcName, formUrl, newWindow) {
            if(newWindow) {
                var caselink = "/servlet/com.sino.sinoflow.servlet.NewCase?sf_projectName=\"" +
                    pjName + "\"&sf_procedureName=\"" +
                    pcName + "\"&sf_url=\"" + formUrl + "\"";
                h = window.screen.height;
                w = window.screen.width;
                f1 = "top=0,left=0,width=" + w + ",height=" + h + ",scrollbars=yes,menubar=yes,toolbar=yes,scroll=yes,resizable=yes";
    ...
    &nbsp;&nbsp;</td>
    <td bordercolordark=white bordercolorlight=white nowrap onClick='openCaseWindow("<%=projName%>","<%=procName%>","<%=procLink%>",<%=isNewWindow%>)' STYLE='cursor:pointer'>
