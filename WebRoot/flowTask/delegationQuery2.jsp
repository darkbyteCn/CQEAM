  ...
  <head>
    ...
    <script type="text/javascript"> 
        ...
        if(<%=hasData%>)
            ...
            ArrActions = new Array(myArrAction0, myArrAction1,myArrAction2 );
        printToolBar();
   ...
   <script type="text/javascript">
  	<!--
	function create(){//新建
		window.location.assign("/servlet/com.sino.sinoflow.servlet.SfDelegationServlet2?act=<%= WebActionConstant.NEW_ACTION %>");
	}
	  	
	function do_ShowDetail(id){//查询详细
		window.location.assign(
		"/servlet/com.sino.sinoflow.servlet.SfDelegationServlet2?act=<%= WebActionConstant.DETAIL_ACTION %>&delegationId="+id);
	}
		...
		for(var i=0;i<tc.length;i++){
			if(tc.item(i).checked){
				if(confirm("确定删除所选项吗？ 继续请点击“确定” 否则点击“取消”")){
					mainFrm.action = "/servlet/com.sino.sinoflow.servlet.SfDelegationServlet2?act=<%= WebActionConstant.DELETE_ACTION %>";
					mainFrm.submit();
					return;
				}else{
					return;
