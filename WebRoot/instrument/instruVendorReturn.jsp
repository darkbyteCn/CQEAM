...
<form action="/servlet/com.sino.ams.instrument.servlet.AmsInstrVendorRetServlet" name="mainForm" method="post">
    ...
    printTitleBar("仪器仪表送修返还页面")
   </script>
    <table border="0" width="100%"  class="queryHeadBg">
        <tr>
            <td align="right" width="10%">单据号：</td>
            <td align="left" width="20%"><input type="text" name="transNo"    style="width:100%" value="<%=StrUtil.nullToString(request.getParameter("transNo"))%>"></td>
            <td  align="right" width="10%">创建日期：</td>
            <td><input type="text" name="fromDate" value="<%=reqParser.getParameter("fromDate")%>" style="width:75%" title="点击选择开始日期" readonly class="readonlyInput"
            ...
            <td><input type="text" name="toDate" value="<%=reqParser.getParameter("toDate")%>"style="width:75%" title="点击选择截止日期" readonly class="readonlyInput"
                                   onclick="gfPop.fEndPop(fromDate, toDate)"><img src="/images/calendar.gif" alt="点击选择截止日期" onclick="gfPop.fEndPop(fromDate, toDate)">
            </td>
    ...
    <input type="hidden" name="act" value="<%=action%>">
      <script type="text/javascript">
        var columnArr = new Array("单据号", "返还人", "确认人","返还日期","确认日期","单据状态");
        ...
        <table width="100%" border="1" bordercolor="#666666" id="dataTab">
            <%
                ...
                for (int i = 0; i < rows.getSize(); i++) {
                    row = rows.getRow(i);
            %>
            <tr class="dataTR" onclick="do_ShowDetail('<%=row.getValue("TRANS_ID")%>')">
...
<script type="text/javascript">
    ...
    function do_Create() {
        var url = "/servlet/com.sino.ams.instrument.servlet.AmsInstrVendorRetServlet?act=<%=WebActionConstant.NEW_ACTION%>" ;
...
//        window.open(url, 'planWin', popscript);
    ...
    function do_ShowDetail(transId) {
        var url = "/servlet/com.sino.ams.instrument.servlet.AmsInstrVendorRetServlet?act=<%=WebActionConstant.DETAIL_ACTION%>&transId=" + transId;
