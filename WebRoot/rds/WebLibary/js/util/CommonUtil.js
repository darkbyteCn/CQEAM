...
function formatCurrency(currencyNum, includeRMB){
    var returnValue = "";
    if(!isNaN(currencyNum)){
        var tmpValue = String(currencyNum);
        var rightStr = "";
        var dotIndex = tmpValue.indexOf(".");
        if(dotIndex > -1){
            rightStr = tmpValue.substring(dotIndex);
            tmpValue = tmpValue.substring(0, dotIndex);
        }
        var charCount = tmpValue.length;
        var addedCount = 0;
        for(var i = charCount- 1; i >= 0; i--){
            returnValue = tmpValue.charAt(i) + returnValue;
            addedCount++;
            if(i > 0){
                if(addedCount % 3 == 0){
