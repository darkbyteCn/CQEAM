...
function CheckSinoButton()
{
	...
	var hasButton=false;
	for(i=0 ;i<nCount;i++){
		if (document.all["sinoDiv"+i].style.display=='' || document.all["sinoDiv"+i].style.display=='block') hasButton=true;
	}
...
function printButton(nPosition,imgFile,desc,tip,jsFun)
{
	if (constIsXPStype)
	{
		...
		if (jsFun.indexOf("(")>0){
			document.write("onClick=\"javascript:"+jsFun+"\" STYLE='cursor:pointer'>\n");
		}else{
			document.write("onClick=\"javascript:"+jsFun+"()\" STYLE='cursor:pointer'>\n");
		}
...
function printViewTitle()
{
	...
	for(i=0;i<nCount;i++)
	{
		if (ArrSinoTitles[i][1]){
			...
			document.write(" onClick=\"javascript:onCheckBoxClick()\" STYLE='cursor:pointer'>\n");
...
columnArr:
widthArr:
tableWidth:   下面TABLE的宽度,默认100%
isScrollX:       是否带x位的SCROLL( 横向滚动条 ), 一般当tableWidth很长的时候设置, 默认false
*/
function printTableHead(columnArr,widthArr,tableWidth, isScrollX,chkBoxName ){
    var scroll_str="overflow-y:scroll";
    if(isScrollX ){
        scroll_str += ";overflow-x:scroll" ;
    } else {
        scroll_str += ";overflow-x:hidden";
    }
    if( null == tableWidth ){
          tableWidth = "100%";
    }
	...
	document.write("<div style=\"left:1px;width:100%;" +scroll_str+ "\" id=\"headDiv\">");
    document.write("<table id=\"headerTable\" border=\"1\" width=\""+tableWidth+"\" class=\""+constTHeadClass+"\" cellpadding=\"0\" cellspacing=\"0\">");
    document.write("<tr height=\"22\">");
    if(columnArr[0] == "checkbox"){
         document.write("<td width=\""+widthArr[0]+"\" align=\"center\"><input type=\"checkBox\" name=\"titleCheck11\" onclick=\"checkAll('titleCheck11','"+chkBoxName+"');\" class=\"headCheckbox\"></td>");
    } else if(columnArr[0] == "radio"){
         document.write("<td width=\""+widthArr[0]+"\" align=\"center\"></td>");
    } else{
	     document.write("<td width=\""+widthArr[0]+"\"  align=\"center\" onclick=\"sortAble('detailTable', 0)\" style=\"cursor:pointer\">"+columnArr[0]+"<img id=\"sortImg0\" src=\"/images/arrow_up_dy.gif\" width=\"11\" height=\"10\"></td>");
    }
    for(var i=1;i<columnArr.length;i++){
        document.write("<td width=\""+widthArr[i]+"\" align=\"center\" onclick=\"sortAble('detailTable', "+i+")\" style=\"cursor:pointer\">"+columnArr[i]+"<img id=\"sortImg"+i+"\" src=\"/images/arrow_up_dy.gif\" width=\"11\" height=\"10\"></td>");
    }
