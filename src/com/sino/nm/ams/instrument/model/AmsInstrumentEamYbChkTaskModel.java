...
import com.sino.nm.ams.instrument.dto.AmsInstrumentEamYbChkTaskDTO;
...
public class AmsInstrumentEamYbChkTaskModel extends AMSSQLProducer {
	...
	/**
     * 功能：仪器仪表检修任务表(AMS) EAM_YB_CHK_TASK 数据库SQL构造层构造函数
	...
	/**
     * 功能：框架自动生成检修任务表(AMS) EAM_YB_CHK_TASK页面翻页查询SQLModel，请根据实际需要修改。
    ...
    public SQLModel getPageQueryModel() {
        ...
        String sqlStr = "";
        	   sqlStr = "SELECT" +
        	   			...
        	   			" dbo.APP_GET_ORGNIZATION_NAME(EYCT.ORGANIZATION_ID) COMPANY" +
        	   			" FROM" +
        	   			" EAM_YB_CHK_TASK EYCT" + 
        	   			" WHERE" +
        	   			" (EYCT.TASK_NAME LIKE ? OR "+ SyBaseSQLUtil.nullStringParam() +")";
    ...
    /**
     * 功能：框架自动生成检修表(AMS) EAM_YB_CHK_TASK数据详细信息查询SQLModel，请根据实际需要修改。
    ...
    /**
     * 功能：框架自动生成检修任务表(AMS) EAM_YB_CHK_TASK数据插入SQLModel，请根据实际需要修改。
    ...
    /**
     * 功能：框架自动生成检修任务表(AMS) EAM_YB_CHK_TASK数据更新SQLModel，请根据实际需要修改。
