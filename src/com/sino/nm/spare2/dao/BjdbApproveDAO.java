...
import com.sino.nm.spare2.model.BjdbModel;
...
//                updateAddressIdOnWay(details);
            ...
            if (fb.isFlowToEnd()) {
                //最后一个节点,则修改单据状态为待出库
                BillUtil.updateStatus("AMS_ITEM_ALLOCATE_H", "TRANS_ID", amsItemTransH.getTransId(), "TRANS_STATUS", SpareConstant.TO_CONSIGN, conn);
            }
            fb.flow();
            conn.commit();
            success = true;
        ...
        if (dtoSet != null && !dtoSet.isEmpty()) {
            ...
            try {
                ...
                for (int i = 0; i < dtoSet.getSize(); i++) {
                    ...
                    cStmt.setString(4, Integer.toString(this.sfUser.getOrganizationId()));
                    cStmt.setInt(5, lineData.getAllocateQty());
                    cStmt.setString(6, Integer.toString(this.sfUser.getUserId()));
