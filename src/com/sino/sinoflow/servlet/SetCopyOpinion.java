...
public class SetCopyOpinion extends BaseServlet {
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        int copyId = StrUtil.strToInt(req.getParameter("copyId"));
        String opinion = util.getReqPara(req, "opinion");
        ...
        try {
            ...
            SQLModel sqlModel = (new SfActCopyModel(user, copy)).setCopyOpinionModel(copyId, opinion);
            DBOperator.updateRecord(sqlModel, conn);
            resout.write("信息保存成功!");
            commitFlag = true;
        } catch (Exception ex) {
            Logger.logError(ex);
            resout.write("[{status:'ERROR', message:'信息保存錯误!'}]");
        } finally {
            if(conn != null) {
                try {
                    if(commitFlag)
