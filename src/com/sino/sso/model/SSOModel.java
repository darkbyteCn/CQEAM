...
public class SSOModel {
    ...
    public static SQLModel getInboxCountModel(String currUserId) {
        String sql =
                "SELECT COUNT(*) TOTAL\n" +
                        "  FROM SF_ACT SA\n" +
                        " WHERE " +
                        "  (CASE WHEN SA.AGENT_USER_ID IS NULL THEN SA.CUR_USERID ELSE SA.AGENT_USER_ID END )=?" +
                        "";
     ...
     * @return
     */
    public static SQLModel getMyWorkModel(int currUserId, String applyNumber, String procName, String actId) {
        SQLModel sm = new SQLModel();
        ArrayList al = new ArrayList();
        String sql = "SELECT SP.USERNAME,\n" +
                "       SP.USER_ID,\n" +
                ...
                "   AND  " + SyBaseSQLUtil.isNotNull("SU.OA_NAME") + " \n" +
                "   AND SA.PROC_ID = SPD.PROC_ID\n" +
                "   AND ((SAI.USER_ID = ISNULL(?, SAI.USER_ID) AND SAI.AGENT_USER_ID  " + SyBaseSQLUtil.isNullNoParam() + " ) OR\n" +
                "       (SAI.AGENT_USER_ID = ISNULL(?, SAI.AGENT_USER_ID)))\n" +
                "   AND (" + SyBaseSQLUtil.isNull() + " OR SPD.PROC_NAME LIKE ?)\n" +
                "   AND (" + SyBaseSQLUtil.isNull() + " OR SA.APPLY_NUMBER LIKE ?)\n" +
     ...
     * @return
     */
    public static SQLModel getMyWorkedModel(int currUserId, String fromDate, String toDate, String applyNum, String procName) {
        SQLModel sm = new SQLModel();
