...
public class SybaseReportCategoryProcessModel extends DefaultRDSBaseSQLModelImpl implements ReportCategoryProcessModel {

    public SybaseReportCategoryProcessModel(BaseUserDTO userAccount, DTO dtoParameter) {
    ...
    public SQLModel getDataUpdateModel() {
        SQLModel sqlModel = new SQLModel();
        String sqlStr = "UPDATE RDS_REPORT_CATEGORY \n" +
                "SET    REPORT_ID         = ?,\n" +
                "       FIELD_DESC        = ?,\n" +
    ...
    public SQLModel getReportCategoryDeleteByIdsModel() {
        SQLModel sqlModel = new SQLModel();
        ReportCategoryFrm frm = (ReportCategoryFrm) dtoParameter;
        String categoryIds = frm.getCategoryIds();
        categoryIds = StrUtil.replaceStr(categoryIds, ",", "', '");
        categoryIds = "'" + categoryIds + "'";
        String sqlStr = "DELETE FROM RDS_REPORT_CATEGORY WHERE CATEGORY_ID IN (" + categoryIds + ")";
        sqlModel.setSqlStr(sqlStr);
        return sqlModel;
    }
    ...
    private SQLModel getDeleteByReportIdModel() {
        SQLModel sqlModel = new SQLModel();
        String sqlStr = "DELETE FROM RDS_REPORT_CATEGORY WHERE REPORT_ID = ?";
        List<String> sqlArgs = new ArrayList<String>();
        ReportCategoryFrm frm = (ReportCategoryFrm) dtoParameter;
        sqlArgs.add(frm.getReportId());
    ...
    public SQLModel getDeleteByViewIdsModel() {
        ...
        List<String> sqlArgs = new ArrayList<String>();
        ReportCategoryFrm frm = (ReportCategoryFrm) dtoParameter;
        String viewIds = frm.getViewIds();
        viewIds = StrUtil.replaceStr(viewIds, ",", "', '");
        viewIds = "'" + viewIds + "'";
        String sqlStr = "DELETE FROM RDS_REPORT_CATEGORY\n" +
                "WHERE  REPORT_ID = ?\n" +
                "       AND EXISTS (SELECT NULL\n" +
                "        FROM   RDS_REPORT_VIEW RRV\n" +
                "        WHERE  REPORT_ID = RRV.REPORT_ID\n" +
                "               AND FIELD_ID = RRV.FIELD_ID\n" +
                "               AND RRV.VIEW_ID IN (" + viewIds + "))";
        sqlArgs.add(frm.getReportId());
