...
public class SybaseModelFieldProcessModel extends DefaultRDSBaseSQLModelImpl implements ModelFieldProcessModel {

    public SybaseModelFieldProcessModel(BaseUserDTO userAccount, DTO dtoParameter) {
    ...
    public SQLModel getDataUpdateModel() {
        SQLModel sqlModel = new SQLModel();
        String sqlStr = "UPDATE RDS_MODEL_FIELD \n" +
                "SET    FIELD_DESC       = ?,\n" +
                "       ENABLED          = ?,\n" +
    ...
    public SQLModel getPageQueryModel() {
        ...
        String sqlStr = "SELECT RMF.FIELD_ID,\n" +
                ...
                "       RMF.FIELD_DESC,\n" +
                "       RDM.DATA_SRC_TYPE,\n" +
                "       dbo.GET_RDS_FLEX_VALUE('DATA_SRC_TYPE', RDM.DATA_SRC_TYPE) DATA_SRC_TYPE_NAME,\n" +
                "       RDM.OWNER,\n" +
                "       RMF.CREATION_DATE,\n" +
                "       RMF.ENABLED,\n" +
                "       dbo.GET_RDS_FLEX_VALUE('ENABLED_FLAG', RMF.ENABLED) ENABLED_NAME\n" +
                "FROM   RDS_MODEL_FIELD RMF,\n" +
                "       RDS_DATA_MODEL  RDM\n" +
                "WHERE  RMF.MODEL_ID = RDM.MODEL_ID\n" +
                "       AND (? = '' OR RMF.FIELD_NAME LIKE ?) \n" +
                "       AND (? = '' OR RMF.ENABLED = ?)\n" +
                "       AND (? = '' OR RDM.MODEL_ID = ?)";
        List<String> sqlArgs = new ArrayList<String>();
        ModelFieldFrm frm = (ModelFieldFrm) dtoParameter;
    ...
    private SQLModel getDataDeleteByModelId() {
        SQLModel sqlModel = new SQLModel();
        String sqlStr = "DELETE FROM RDS_MODEL_FIELD WHERE MODEL_ID = ?";
        List<String> sqlArgs = new ArrayList<String>();
        ModelFieldFrm frm = (ModelFieldFrm) dtoParameter;
        sqlArgs.add(frm.getModelId());
        sqlModel.setSqlStr(sqlStr);
