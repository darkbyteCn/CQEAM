...
public class OracleDynamicSQLProduceModel extends DefaultRDSBaseSQLModelImpl implements DynamicSQLProduceModel {

    private ReportExecuteFrm executeFrm = null;
    private SearchParameterFrm searchFrm = null;


    public OracleDynamicSQLProduceModel(BaseUserDTO userAccount, DTO dtoParameter) {
                    ...
                    for (int i = 0; i < containNumber; i++) {
                        ...
                        if (index == 0) {//如果是Session变量，则需要检查用户类中是否含有指定属性
                            ...
                            if (RDSConstantConfigManager.contains(userProp)) {
                                ...
                                sqlArgs.add(ReflectionUtil.getProperty(userAccount, userProp));
                            }
                        } else {//非Session变量
                            parameterStr = "{" + parameterStr + "}";
                            sqlStr = StrUtil.replaceFirst(sqlStr, parameterStr, "?");
                            String parameterValue = searchFrm.getParameter(StrUtil.getJavaField(fieldName));
