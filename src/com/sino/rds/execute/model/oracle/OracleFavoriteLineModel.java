...
public class OracleFavoriteLineModel extends DefaultRDSBaseSQLModelImpl implements FavoriteLineModel {

    public OracleFavoriteLineModel(BaseUserDTO userAccount, DTO dtoParameter) {
    ...
    public SQLModel getFavoriteLineCreateModel(String[] reportIdArr) {
        ...
        String sqlStr = "INSERT INTO RDS_FAVORITE_LINE\n" +
                ...
                "   HEADER_ID,\n" +
                "   REPORT_ID)\n" +
                "  (SELECT RDS_FAVORITE_LINE_S.NEXTVAL,\n" +
                "          ?,\n" +
                "          RRD.REPORT_ID\n" +
        ...
        reportIds = reportIds.substring(0, reportIds.length() - 2);
    ...
    private SQLModel getDeleteByHeaderIdModel() {
        SQLModel sqlModel = new SQLModel();
        String sqlStr = "DELETE FROM RDS_FAVORITE_LINE RFL WHERE RFL.HEADER_ID = ?";
        List<String> sqlArgs = new ArrayList<String>();
        FavoriteLineFrm frm = (FavoriteLineFrm) dtoParameter;
    ...
    public SQLModel getDeleteByPrimaryKeyModel() {
        SQLModel sqlModel = new SQLModel();
        String sqlStr = "DELETE FROM RDS_FAVORITE_LINE RFL WHERE RFL.LINE_ID = ?";
        List<String> sqlArgs = new ArrayList<String>();
        FavoriteLineFrm frm = (FavoriteLineFrm) dtoParameter;
