package com.sino.soa.mis.srv.assettransincompanysrv.servlet;
 ...
 */
public class SBFIFAAssetsTransInCompanyServlet extends BaseServlet {
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			req2DTO.setDTOClassName(SBFIFAAssetsTransInCompanyDTO.class.getName());
			SBFIFAAssetsTransInCompanyDTO dto = (SBFIFAAssetsTransInCompanyDTO) req2DTO.getDTO(req);
			conn = getDBConnection(req);
			SBFIFAAssetsTransInCompanyDAO commitDAO = new SBFIFAAssetsTransInCompanyDAO(user, dto, conn);
			...
			if (action.equals("")) {
				...
				forwardURL = SrvURLDefineList.ASSETS_COMMITS;
			} else if (action.equals(SrvWebActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new SBFIFAAssetsTransInCompanyModel(user, dto);
				...
				List list = Arrays.asList(systemIds);
				Set set = new HashSet(list);
				String [] rid=(String [])set.toArray(new String[0]);
                String systemId = arrToStr(rid, ",");
