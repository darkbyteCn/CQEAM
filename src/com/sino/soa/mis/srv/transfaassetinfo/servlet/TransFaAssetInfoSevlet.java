package com.sino.soa.mis.srv.transfaassetinfo.servlet;
...
public class TransFaAssetInfoSevlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            if (action.equals("")) {
                forwardURL = SrvURLDefineList.TRANS_FAASSET_PAGE;
                ...
                TransFaAssetInfoSrv service = new TransFaAssetInfoSrv();
...
//                service.setPeriodName("NOV-11");
//                //此处需要增加资产账簿0217
//                service.setBookTypeCode("HNMC_FA_2521");
                service.excute();
                ...
                if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    logDTO.setSynMsg("同步'MIS资产财务信息(ODI)'成功, 耗时" + resumeTime + "毫秒, 会计期间:"+dtoParameter.getPeriodName());
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_FA_TRANS_ASSET, conn);
                    message.setMessageValue("同步'MIS资产财务信息(ODI)'成功, 耗时" + resumeTime + "毫秒");
                } else {
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_FA_TRANS_ASSET);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步'MIS资产财务信息(ODI)'失败, 耗时" + resumeTime + "毫秒, 会计期间:"+dtoParameter.getPeriodName()+", 错误信息:"+srvMessage.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    message.setMessageValue("同步'MIS资产财务信息(ODI)'失败，" + "耗时" + resumeTime + "毫秒, 错误信息:"+srvMessage.getErrorMessage());
                }
                forwardURL = SrvURLDefineList.TRANS_FAASSET_PAGE;
            } 
