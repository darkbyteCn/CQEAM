package com.sino.soa.mis.srv.projectInfo.servlet;
...
public class SrvProjectInfoServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        String forwardURL = com.sino.soa.common.SrvURLDefineList.SRV_PROJECT_PAGE;
        ...
        try {
            ...
            req2DTO.setDTOClassName(SrvProjectInfoDTO.class.getName());
            SrvProjectInfoDTO dtoParameter = (SrvProjectInfoDTO) req2DTO.getDTO(req);
            ...
            SrvProjectInfoDAO srvProjectInfoDAO = new SrvProjectInfoDAO(user, dtoParameter, conn);
            ...
            if (action.equals("")) {
                ...
                forwardURL = SrvURLDefineList.SRV_PROJECT_PAGE;
            } else if (action.equals(WebActionConstant.SYSCHRONIZE_ACTION) && !AssetsWebAttributes.TD_ASSETS_TYPE.equals(assetsType)) {
                ...
                logDTO.setSynMsg("同步项目信息开始");
                logUtil.synLog(logDTO, conn);
                InquiryProjectInfoSrv projectInfoSrv = new InquiryProjectInfoSrv();   //调用接口
                projectInfoSrv.setSegment1(dtoParameter.getSegment1());   //项目编号
                ...
                projectInfoSrv.setOrgCode(dtoParameter.getOrgCode());     //公司编号
                ...
                if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    for (int i = 0; i < ds.getSize(); i++) {
                        ...
                        srvProjectInfoDAO = new SrvProjectInfoDAO(user, dto, conn);       
                        try {  	
                    ...
                    logDTO.setSynType(SrvType.SRV_PA_PROJECT);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步项目信息结束同步" + (totalCount + errorCount) + "条记录，成功" + totalCount + "，失败" + errorCount + "，耗时" + resumeTime + "毫秒，公司及项目编号:"+dtoParameter.getOrgCode()+","+dtoParameter.getSegment1());
                    logUtil.synLog(logDTO, conn);
                    message = new Message();
                    message.setMessageValue("同步项目信息" + (totalCount + errorCount) + "条记录，成功" + totalCount + "，失败" + errorCount + "，耗时" + resumeTime + "毫秒");
                } else {
                    resumeTime = System.currentTimeMillis() - start;
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_PA_PROJECT);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步项目信息失败，耗时" + resumeTime + "毫秒，公司及项目编号:"+dtoParameter.getOrgCode()+","+dtoParameter.getSegment1()+"。错误信息："+srvMessage.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    message = new Message();
                    message.setMessageValue("同步项目信息失败，耗时" + resumeTime + "毫秒，错误信息："+srvMessage.getErrorMessage());
                }
