        ...
        try {
            ...
            if (action.equals("")) {
                ...
                if (srm.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    logDTO.setSynType(SrvType.SRV_BOOK);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步MIS资产帐簿结束, 同步" + (count + falsecount) + "条记录，成功" + count + "，失败" + (falsecount) + "，耗时" + resumeTime + "毫秒。资产账簿："+bookTypeCode);
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_BOOK, conn);
                    message = new Message();
                    message.setMessageValue("同步MIS资产帐簿 " + (count + falsecount) + "条记录，成功" + count + "，失败" + (falsecount) + "，耗时" + resumeTime + "毫秒");
                } else{
                	 resumeTime = System.currentTimeMillis() - start;
                     logDTO = new SynLogDTO();
                     logDTO.setSynType(SrvType.SRV_BOOK);
                     logDTO.setCreatedBy(user.getUserId());
                     logDTO.setSynMsg("同步MIS资产帐簿失败, 耗时" + resumeTime + "毫秒, 资产账簿："+bookTypeCode+"。 错误信息:"+srm.getErrorMessage());
                     logUtil.synLog(logDTO, conn);
                     message = new Message();
                     message.setMessageValue("同步MIS资产帐簿失败, 耗时" + resumeTime + "毫秒, 资产账簿："+bookTypeCode+"。 错误信息:"+srm.getErrorMessage());
                }
                req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.SRV_ESSET_BOOK_PAGE;
            }
