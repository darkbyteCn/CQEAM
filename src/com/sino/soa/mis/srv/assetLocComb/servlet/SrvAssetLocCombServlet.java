package com.sino.soa.mis.srv.assetLocComb.servlet;
...
* function：资产地点组合批量导入
*/
public class SrvAssetLocCombServlet extends BaseServlet{
	 ...
	 public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
	        ...
	        try {
	            ...
	            req2DTO.setDTOClassName(SrvAssetLocCombDTO.class.getName());
	            SrvAssetLocCombDTO dto = (SrvAssetLocCombDTO) req2DTO.getDTO(req);
	            SrvAssetLocCombDAO eamNewLocusDAO = new SrvAssetLocCombDAO(user, dto, conn);
	            OrgOptionProducer optionProducer=new OrgOptionProducer(user,conn);
//	            dto.setOrganizationOpt(optionProducer.getOrgnizationOption(dto.getOrganizationId(), "N"));
	            dto.setOrganizationOpt(optionProducer.getSynOrgnizationOption(dto.getOrganizationId(), "N"));
	            if (action.equals("")) {
	                req.setAttribute(WebAttrConstant.SYSCHRONIZE_DTO, dto);
	                forwardURL = SrvURLDefineList.SRV_ASSET_LOC_COMB_PAGE;
	            } else if (action.equals(WebActionConstant.QUERY_ACTION)) { //查询操作
	                BaseSQLProducer sqlProducer = new SrvAssetLocCombModel(user, dto);
	                ...
	                forwardURL = "/servlet/com.sino.soa.mis.srv.assetLocComb.servlet.SrvAssetLocCombServlet?act=QUERY_ACTION";   //
	            } else {
