package com.sino.soa.mis.srv.InquiryRetiredAssetDetail.model;
...
public class PageRetiredAssetDetailModel extends BaseSQLProducer {
     ...
     */
    public PageRetiredAssetDetailModel(SfUserDTO userAccount, PageRetiredAssetDTO dtoParameter) {
    ...
    public SQLModel getDataCreateModel() {
        ...
        int assetId = -1;
        if (!pageRetiredAssetDTO.getAssetId().equals("")) {
        	assetId = Integer.parseInt(pageRetiredAssetDTO.getAssetId()); 
        }
        sqlArgs.add(assetId);
        ...
        sqlArgs.add(pageRetiredAssetDTO.getCostRetired());
    ...
    public SQLModel getDataUpdateModel() {
        ...
        String sqlStr = "UPDATE "
                + " ZTE_FA_RETIREMENT_BASIC_INFO SET"
                + " BOOK_TYPE_CODE=?,"
                + " ASSET_ID=?,"
                + " ASSET_NUMBER=?,"
                + " TAG_NUMBER=?,"
    ...
    public SQLModel getAssetExistsModel() {
        ...
        String sqlStr = "SELECT"
                + "	ECOM.ASSET_ID"
                + "	FROM ZTE_FA_RETIREMENT_BASIC_INFO ECOM WHERE ECOM.ASSET_ID=?";
        sqlModel.setSqlStr(sqlStr);
