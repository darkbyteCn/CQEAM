package com.sino.soa.mis.srv.valueinfo.servlet;
 ...
 */
public class SBSYSYInquiryVSetValueInfoServlet extends BaseServlet {

    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        String forwardURL = SrvURLDefineList.SRV_SETVALUE_PAGE;
        ...
        try {
            ...
            req2DTO.setDTOClassName(SBSYSYInquiryVSetValueInfoDTO.class.getName());
            SBSYSYInquiryVSetValueInfoDTO dtoParameter = (SBSYSYInquiryVSetValueInfoDTO) req2DTO.getDTO(req);
            String action = dtoParameter.getAct();
            String assetType = StrUtil.nullToString(req.getParameter("type"));
            conn = getDBConnection(req);
            SBSYSYInquiryVSetValueInfoDAO mFndFlexValuesDAO = new SBSYSYInquiryVSetValueInfoDAO(user, dtoParameter, conn);
            OptionProducer optionProducer = new OptionProducer(user, conn);
            String range = "MIS";//非TD
            ...
            if (action.equals("")) {
                ...
                String lastUpdateDate = SynUpdateDateUtils.getLastUpdateDate(SrvType.SRV_SET_VALUESET, conn);
                dtoParameter.setLastUpdateDate(lastUpdateDate);
                req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.SRV_SETVALUE_PAGE;
                ...
                if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    for (int i = 0; i < ds.getSize(); i++) {
                        SBSYSYInquiryVSetValueInfoDTO dto = (SBSYSYInquiryVSetValueInfoDTO) ds.getDTO(i);
                        mFndFlexValuesDAO = new SBSYSYInquiryVSetValueInfoDAO(user, dto, conn);
                        try {
                    ...
                    logDTO.setSynType(SrvType.SRV_SET_VALUESET);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步MIS值集值信息服务结束, 同步" + (totalCount + errorCount) + "条记录, 成功" + totalCount + ", 失败" + errorCount + ", 耗时" + resumeTime + "毫秒");
                    logUtil.synLog(logDTO, conn);
                    message = new Message();
                    message.setMessageValue("同步MIS值集值信息服务结束, 同步" + (totalCount + errorCount) + "条记录, 成功" + totalCount + ", 失败" + errorCount + ", 耗时" + resumeTime + "毫秒");
                }else{
                	  resumeTime = System.currentTimeMillis() - start;
                      logDTO = new SynLogDTO();
                      logDTO.setSynType(SrvType.SRV_SET_VALUESET);
                      logDTO.setCreatedBy(user.getUserId());
                      logDTO.setSynMsg("同步MIS值集值信息服务失败, 耗时" + resumeTime + "毫秒, 值集名称："+dtoParameter.getFlexValueName()+"。错误信息:"+srvMessage.getErrorMessage());
                      logUtil.synLog(logDTO, conn);
                      message = new Message();
                      message.setMessageValue("同步MIS值集值信息服务失败, 耗时" + resumeTime + "毫秒, 值集名称："+dtoParameter.getFlexValueName()+"。错误信息:"+srvMessage.getErrorMessage());
                }
                req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.SRV_SETVALUE_PAGE;
            } else {
