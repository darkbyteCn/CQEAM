package com.sino.soa.td.srv.assetcategory.servlet;
 ...
 */
public class SBFIFATdSrvAssetCategoryServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            req2DTO.setDTOClassName(SBFIFATdSrvAssetCategoryDTO.class.getName());
            SBFIFATdSrvAssetCategoryDTO dtoParameter = (SBFIFATdSrvAssetCategoryDTO) req2DTO.getDTO(req);
            String action = dtoParameter.getAct();
            String assetsType = StrUtil.nullToString(req.getParameter("assetsType"));
            dtoParameter.setAssetsType(assetsType);
            conn = getDBConnection(req);
            SBFIFATdSrvAssetCategoryDAO srvAssetCategoryDAO = new SBFIFATdSrvAssetCategoryDAO(user, dtoParameter, conn);
            ...
            if (action.equals("")) {
                forwardURL = SrvURLDefineList.TD_SRV_ESSET_CATEGORY_PAGE;
                ...
                if (srm.getErrorFlag().equalsIgnoreCase("Y")) {
                    SynUpdateDateUtils.createLastUpdateDate(SrvType.SRV_TD_FA_CATEGORY, conn);
                    DTOSet ds = service.getDs();
                    if (ds.getSize() > 0) {
                    ...
                    logDTO.setSynType(SrvType.SRV_TD_FA_CATEGORY);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步TD资产类别结束,同步" + totalCont + "条记录，成功" + count + "，失败" + falsecount + "，耗时" + resumeTime + "毫秒。资产账簿："+dtoParameter.getBookTypeCode());
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_TD_FA_CATEGORY, conn);
                    message = new Message();
                    message.setMessageValue("同步TD资产类别 " + totalCont + "条记录，成功" + count + "，失败" + falsecount + "，耗时" + resumeTime + "毫秒");
                } else {
                    resumeTime = System.currentTimeMillis() - start;
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_TD_FA_CATEGORY);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步TD资产类别失败, 耗时" + resumeTime + "毫秒,资产账簿："+dtoParameter.getBookTypeCode()+"。错误信息："+srm.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    message = new Message();
                    message.setMessageValue("同步TD资产类别失败, 耗时" + resumeTime + "毫秒,资产账簿："+dtoParameter.getBookTypeCode()+"。错误信息："+srm.getErrorMessage());
                }  
                req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.TD_SRV_ESSET_CATEGORY_PAGE;
