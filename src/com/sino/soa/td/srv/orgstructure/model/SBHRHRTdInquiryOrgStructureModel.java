package com.sino.soa.td.srv.orgstructure.model;
 ...
 */
public class SBHRHRTdInquiryOrgStructureModel extends BaseSQLProducer {

	private SfUserDTO sfUser = null;

	public SBHRHRTdInquiryOrgStructureModel(SfUserDTO userAccount, SBHRHRTdInquiryOrgStructureDTO dtoParameter) {
	...
	public SQLModel getDataCreateModel() {
		SQLModel sqlModel = new SQLModel();
		List sqlArgs = new ArrayList();
		SBHRHRTdInquiryOrgStructureDTO srvAssetBookDTO = (SBHRHRTdInquiryOrgStructureDTO)dtoParameter;
...
//		sqlArgs.add(srvAssetBookDTO.getOrganizationCode());
    ...
    public SQLModel getSinoDeptCreateModel() {
		...
		String sqlStr = "INSERT INTO SINO_MIS_DEPT\n" +
                ...
                " SECOND_DEPT,\n" +
                " SPECIALITY_DEPT)\n" +
                "(SELECT ?, ?, (SELECT ISNULL((SELECT EO.ORGANIZATION_ID FROM ETS_OU_CITY_MAP  EO  WHERE EO.COMPANY_CODE=?),-1)), 'Y', GETDATE(), ?, ?, ?, ?, 'N', 'N')";
        sqlArgs.add(srvAssetBookDTO.getOrganizationId());
		sqlArgs.add(srvAssetBookDTO.getOrganizationName());
        sqlArgs.add(srvAssetBookDTO.getOrganizationCode());
        sqlArgs.add(srvAssetBookDTO.getOrganizationCode());
