package com.sino.soa.td.srv.orgstructure.servlet;
 ...
 */
public class SBHRHRTdInquiryOrgStructureServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            req2DTO.setDTOClassName(SBHRHRTdInquiryOrgStructureDTO.class.getName());
            SBHRHRTdInquiryOrgStructureDTO dtoParameter = (SBHRHRTdInquiryOrgStructureDTO) req2DTO.getDTO(req);
            String action = dtoParameter.getAct();
            conn = getDBConnection(req);
            SBHRHRTdInquiryOrgStructureDAO dao = new SBHRHRTdInquiryOrgStructureDAO(user, dtoParameter, conn);
            req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
            if (action.equals("")) {
                forwardURL = SrvURLDefineList.TD_SRV_ORG_STRUCTURE_PAGE;
                ...
                if (srm.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    message = new Message();
                    message.setMessageValue("同步TD组织结构" + (count + falsecount) + "条记录，成功" + count + "，失败" + (falsecount) + "，耗时" + resumeTime + "毫秒");
                }else{
                    resumeTime = System.currentTimeMillis() - start;
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_TD_ORG_STRUCTURE);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步TD组织结构失败, 耗时" + resumeTime + "毫秒。错误信息:"+srm.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    message = new Message();
                    message.setMessageValue("同步TD组织结构失败, 耗时" + resumeTime + "毫秒。错误信息:"+srm.getErrorMessage());
                }
                req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.TD_SRV_ORG_STRUCTURE_PAGE;
            }
