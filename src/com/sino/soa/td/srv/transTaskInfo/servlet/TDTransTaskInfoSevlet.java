...
public class TDTransTaskInfoSevlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            if (action.equals("")) {
                forwardURL = SrvURLDefineList.TD_TRANS_TASKINFO_PAGE;
                ...
                String envCode = SynUpdateDateUtils.getEnvCode("TDTransTaskInfoSrv", conn);  //环境代码待确认
                TDTransTaskInfoSrv service = new TDTransTaskInfoSrv();
                ...
                if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    logDTO.setSynMsg("同步'TD系统项目任务信息(ODI)'成功, 耗时" + resumeTime + "毫秒, 项目编号:"+dtoParameter.getProjectNum());
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_TD_PA_TASK, conn);
                    message.setMessageValue("同步'TD系统项目任务信息(ODI)'成功，" + "耗时" + resumeTime + "毫秒");
                } else {
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_TD_PA_TASK);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步'TD系统项目任务信息(ODI)'失败, 耗时" + resumeTime + "毫秒, 项目编号:"+dtoParameter.getProjectNum()+", 错误信息:"+srvMessage.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    message.setMessageValue("同步'TD系统项目任务信息(ODI)'失败, 耗时" + resumeTime + "毫秒, 错误信息:"+srvMessage.getErrorMessage());
                }
                forwardURL = SrvURLDefineList.TD_TRANS_TASKINFO_PAGE;
            }
