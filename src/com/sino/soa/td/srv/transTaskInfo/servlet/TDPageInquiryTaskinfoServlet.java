package com.sino.soa.td.srv.transTaskInfo.servlet;
...
import com.sino.soa.td.srv.transTaskInfo.dao.TDPageInquiryTaskinfoDAO;   
import com.sino.soa.td.srv.transTaskInfo.srv.TDPageInquiryTaskInfoSrv;      
...
public class TDPageInquiryTaskinfoServlet extends BaseServlet {

	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		String forwardURL = SrvURLDefineList.TD_SRV_PROJECTTASK1_PAGE;
		...
		try {
			...
			if (action.equals("")) {
				...
				forwardURL = SrvURLDefineList.TD_SRV_PROJECTTASK1_PAGE;
				...
				logDTO.setSynMsg("同步TD项目任务信息开始");
				logUtil.synLog(logDTO, conn);
				
				TDPageInquiryTaskInfoSrv projectTaskInfoSrv = new TDPageInquiryTaskInfoSrv();
				...
				if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
					...
					for (int i = 0; i < ds.getSize(); i++) {
						...
						TDPageInquiryTaskinfoDAO srvTaskinfoDAO = new TDPageInquiryTaskinfoDAO(user, dto, conn);
					    try {
							if(SynUpdateDateUtils.getBetweenDays(SynUpdateDateUtils.getLastUpdateDate(SrvType.SRV_TD_PA_TASK, conn),(dto.getLastUpdateDate().toString()))>0){
