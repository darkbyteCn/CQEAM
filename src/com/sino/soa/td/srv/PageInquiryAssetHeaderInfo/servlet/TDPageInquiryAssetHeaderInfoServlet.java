package com.sino.soa.td.srv.PageInquiryAssetHeaderInfo.servlet;
...
import com.sino.soa.td.srv.PageInquiryAssetHeaderInfo.dao.TDPageInquiryAssetHeaderInfoDAO; //PageInquiryAssetHeaderInfoDAO;   
import com.sino.soa.td.srv.PageInquiryAssetHeaderInfo.srv.TDPageInquiryAssetHeaderInfoSrv; //PageInquiryAssetHeaderInfoSrv;   
 ...
 * user:wangzp
 * function:查询资产头基本信息（分页）_TD 
 */

public class TDPageInquiryAssetHeaderInfoServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            TDPageInquiryAssetHeaderInfoDAO retiredAssetDetailDAO = new TDPageInquiryAssetHeaderInfoDAO(user, dtoParameter, conn);
            AssetsOptProducer optProducer = new AssetsOptProducer(user, conn);
            String opt = optProducer.getBookTypeOption(dtoParameter.getBookTypeCode());
            dtoParameter.setOrgOption(opt);
            if (action.equals("")) {
            	req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.TD_SRV_ASSET_HEADER_PAGE;
            } else if (action.equals(SrvWebActionConstant.QUERY_ACTION)) {
            	TDPageInquiryAssetHeaderInfoSrv service = new TDPageInquiryAssetHeaderInfoSrv();
                ...
                boolean isSyn=retiredAssetDetailDAO.assetHeaderODI_TD();
        	     message=new Message();
              if(isSyn){
                  message.setMessageValue("调用'资产头信息_TD'存储过程成功!"); 
              }else{
           	   message.setMessageValue("调用'资产头信息_TD'存储过程失败！请检查!"); 
              }
              	req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
               forwardURL = SrvURLDefineList.TD_SRV_ASSET_HEADER_PAGE;
           }
