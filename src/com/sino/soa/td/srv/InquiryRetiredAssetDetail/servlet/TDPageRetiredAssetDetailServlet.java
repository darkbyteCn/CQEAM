package com.sino.soa.td.srv.InquiryRetiredAssetDetail.servlet;
...
import com.sino.soa.td.srv.InquiryRetiredAssetDetail.dao.TDPagRetiredAssetDetailDAO;
import com.sino.soa.td.srv.InquiryRetiredAssetDetail.srv.TDPageInquiryRetiredAssetDetailSrv;
...
public class TDPageRetiredAssetDetailServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            TDPagRetiredAssetDetailDAO retiredAssetDetailDAO = new TDPagRetiredAssetDetailDAO(user, dtoParameter, conn);
            AssetsOptProducer optProducer = new AssetsOptProducer(user, conn);
            String opt = optProducer.getBookTypeOption(dtoParameter.getBookTypeCode());
            dtoParameter.setOrgOption(opt);
            if (action.equals("")) {
            	req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.TD_SRV_RETIRED_ASSET_DETAIL1_PAGE;
            } else if (action.equals(SrvWebActionConstant.QUERY_ACTION)) {
                TDPageInquiryRetiredAssetDetailSrv service = new TDPageInquiryRetiredAssetDetailSrv();
                ...
                if (srm.getErrorFlag().equals("Y")) {
                    ...
                    logDTO.setSynType(SrvType.SRV_TD_FA_RETIRE);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步读取TD报废资产结束, 同步"+(errerCount+count)+"条记录，成功"+count+"，失败"+(errerCount)+"，耗时"+resumeTime+"毫秒, 资产账簿："+dtoParameter.getBookTypeCode());
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_TD_FA_RETIRE, conn);
                    message=new Message();
                    message.setMessageValue("同步读取TD报废资产 "+(errerCount+count)+"条记录，成功"+count+"，失败"+(errerCount)+"，耗时"+resumeTime+"毫秒");
                } else {
                	resumeTime = System.currentTimeMillis() - start;
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_TD_FA_RETIRE);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步读取TD报废资产失败, 耗时"+resumeTime+"毫秒, 资产账簿："+dtoParameter.getBookTypeCode()+"。错误信息:"+srm.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    message=new Message();
                    message.setMessageValue("同步读取TD报废资产失败, 耗时"+resumeTime+"毫秒,, 资产账簿："+dtoParameter.getBookTypeCode()+"。错误信息:"+srm.getErrorMessage());
                }
                req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.TD_SRV_RETIRED_ASSET_DETAIL1_PAGE;
            }
