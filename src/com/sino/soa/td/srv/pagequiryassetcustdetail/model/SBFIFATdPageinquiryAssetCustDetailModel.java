package com.sino.soa.td.srv.pagequiryassetcustdetail.model;
 ...
 */
public class SBFIFATdPageinquiryAssetCustDetailModel extends BaseSQLProducer {

	private SfUserDTO sfUser = null;

	public SBFIFATdPageinquiryAssetCustDetailModel(SfUserDTO userAccount, SBFIFATdPageinquiryAssetCustDetailDTO dtoParameter) {
    ...
    public SQLModel getEcouInforModel(String projectAssetId, String taskId, String projectId) {
        ...
        String sqlStr = "SELECT * FROM ZTE_TD_CUST_DETAIL ZFCD WHERE ZFCD.PROJECT_ASSET_ID = ? AND ZFCD.TASK_ID = ? AND ZFCD.PROJECT_ID = ?";
	...
	public SQLModel getDataCreateModel() throws SQLModelException {
		SQLModel sqlModel = new SQLModel();
		List sqlArgs = new ArrayList();
		SBFIFATdPageinquiryAssetCustDetailDTO srvAssetCategory = (SBFIFATdPageinquiryAssetCustDetailDTO)dtoParameter;
        String sqlStr = "INSERT INTO "
			+ " ETS_TD_CUST_DETAIL_IMP("
			+ " TAG_NUMBER,"
			+ " ASSET_NAME,"
            + " ASSET_DESCRIPTION,"
            + " MODEL_NUMBER,"
	...
	public SQLModel getDataUpdateModel() throws SQLModelException {
		...
		List sqlArgs = new ArrayList();
		SBFIFATdPageinquiryAssetCustDetailDTO srvAssetCategory = (SBFIFATdPageinquiryAssetCustDetailDTO)dtoParameter;
		String sqlStr = "UPDATE ZTE_TD_CUST_DETAIL"
                    + " SET"
                    + " BOOK_TYPE_CODE = ?,"
                    + " PROJECT_NUMBER = ?,"
                    + " TASK_NUMBER = ?,"
...
//        sqlArgs.add(srvAssetCategory.getLastUpdateDate());
    ...
    public SQLModel getSyncTotalCountModel(String projectNum) {
        ...
        String sqlStr = "SELECT COUNT(1) SYNC_TOTAL_COUNT \n" +
                "FROM ETS_TD_CUST_DETAIL_IMP EFCDI \n" +
                "WHERE EFCDI.CREATE_USER_ID = ? \n" +
                "AND EFCDI.PROJRCT_NUMBER= ? \n" +
    ...
    public SQLModel getSyncErrorModel() {
		SQLModel sqlModel = new SQLModel();
	    List sqlArgs = new ArrayList();
        String sqlStr = "SELECT COUNT(1) SYNC_ERROR_COUNT FROM ETS_AUTO_SYN_LOG EASL WHERE EASL.SYN_TYPE = 'SYN_CUST_DETAIL_TD' AND EASL.CREATED_BY = ?";
		sqlArgs.add(sfUser.getUserId());
        sqlModel.setSqlStr(sqlStr);
		sqlModel.setArgs(sqlArgs);
		return sqlModel;
    ...
    */
    public SQLModel getDataImpDeleteModel(String projectNum) throws SQLModelException {
		SQLModel sqlModel = new SQLModel();
		List sqlArgs = new ArrayList();
		String sqlStr = "DELETE FROM ETS_TD_CUST_DETAIL_IMP WHERE CREATE_USER_ID = ? AND  PROJRCT_NUMBER= ? ";
        sqlArgs.add(sfUser.getUserId());
        sqlArgs.add(projectNum);
		sqlModel.setSqlStr(sqlStr);
		sqlModel.setArgs(sqlArgs);
    ...
    public SQLModel getDataLogDeleteModel() throws SQLModelException {
		SQLModel sqlModel = new SQLModel();
		List sqlArgs = new ArrayList();
		String sqlStr = "DELETE FROM ETS_AUTO_SYN_LOG WHERE SYN_TYPE = 'SYN_CUST_DETAIL_TD' AND CREATED_BY = ?";
        sqlArgs.add(sfUser.getUserId());
    ...
    public SQLModel getErrorRowModel() {
        ...
        String sqlStr = "SELECT EASL.SYN_TYPE,\n" +
                        ...
                        " WHERE EASL.SYN_TYPE = 'SYN_CUST_DETAIL_TD'\n" +
