package com.sino.soa.td.srv.assettransbtwcompanysrv.servlet;
 ...
 */
public class SBFIFATdAssetsTransBtwCompanyServlet extends BaseServlet {
	/**
	 *
	 */
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			req2DTO.setDTOClassName(SBFIFATdAssetsTransBtwCompanyDTO.class.getName());
			SBFIFATdAssetsTransBtwCompanyDTO dto = (SBFIFATdAssetsTransBtwCompanyDTO) req2DTO.getDTO(req);
			conn = getDBConnection(req);
			SBFIFATdAssetsTransBtwCompanyDAO commitDAO = new SBFIFATdAssetsTransBtwCompanyDAO(user, dto, conn);
			ServletConfigDTO servletConfig = getServletConfig(req);
			if (action.equals("")) {
				req.setAttribute(QueryConstant.QUERY_DTO, dto);
				forwardURL = SrvURLDefineList.TD_ASSETS_BTW_COMMITS;
			} else if (action.equals(SrvWebActionConstant.QUERY_ACTION)) { //查询操作
				BaseSQLProducer sqlProducer = new SBFIFATdAssetsTransBtwCompanyModel(user, dto);
