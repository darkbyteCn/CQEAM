package com.sino.soa.td.srv.PageInquiryAssetDistribution.servlet;
...
import com.sino.soa.td.srv.PageInquiryAssetDistribution.dao.TDPageInquiryAssetDistributionDAO;    
import com.sino.soa.mis.srv.PageInquiryAssetDistribution.dto.InquiryAssetDistributionDTO;     
import com.sino.soa.td.srv.PageInquiryAssetDistribution.srv.TDPageInquiryAssetDistributionSrv; 
...
public class TDPageInquiryAssetDistributionServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            req2DTO.setDTOClassName(InquiryAssetDistributionDTO.class.getName());
            InquiryAssetDistributionDTO dtoParameter = (InquiryAssetDistributionDTO) req2DTO.getDTO(req);
            String action = dtoParameter.getAct();
            String assetsType = req.getParameter("assetsType");
            conn = getDBConnection(req);
            dtoParameter.setAssetsType(assetsType);
            TDPageInquiryAssetDistributionDAO retiredAssetDetailDAO = new TDPageInquiryAssetDistributionDAO(user, dtoParameter, conn);
            AssetsOptProducer optProducer = new AssetsOptProducer(user, conn);
            String opt = optProducer.getBookTypeOption(dtoParameter.getBookTypeCode());
            dtoParameter.setOrgOption(opt);
            if (action.equals("")) {
            	req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.TD_SRV_ASSET_DISTRIBUTION_PAGE;
            } else if (action.equals(SrvWebActionConstant.QUERY_ACTION)) {
            	TDPageInquiryAssetDistributionSrv service = new TDPageInquiryAssetDistributionSrv();
                service.setBookTypeCode(dtoParameter.getBookTypeCode());   //资产账簿
                service.setAssetNumber(dtoParameter.getAssetNumber());     //资产编号      
               ...
               boolean isSyn=retiredAssetDetailDAO.assetDistributionODI_TD();
         	    message=new Message();
               if(isSyn){
                   message.setMessageValue("调用'资产分配行信息_TD'存储过程成功!"); 
               }else{
            	   message.setMessageValue("调用'资产分配行信息_TD'存储过程失败！请检查!"); 
               }
           	    req.setAttribute(QueryConstant.QUERY_DTO, dtoParameter);
                forwardURL = SrvURLDefineList.TD_SRV_ASSET_DISTRIBUTION_PAGE;
