package com.sino.soa.td.srv.pagequiryassetdeprecation.servlet;
...
import com.sino.soa.td.srv.pagequiryassetdeprecation.dao.SBFIFATdPageQuiryAssetDeprecationDAO;
import com.sino.soa.td.srv.pagequiryassetdeprecation.dto.SBFIFATdPageQuiryAssetDeprecationDTO;
import com.sino.soa.td.srv.pagequiryassetdeprecation.srv.SBFIFATdPageQuiryAssetDeprecationSrv;
 ...
 */
public class SBFIFATdPageQuiryAssetDeprecationServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            req2DTO.setDTOClassName(SBFIFATdPageQuiryAssetDeprecationDTO.class.getName());
            SBFIFATdPageQuiryAssetDeprecationDTO dtoParameter = (SBFIFATdPageQuiryAssetDeprecationDTO) req2DTO.getDTO(req);
            String action = dtoParameter.getAct();
            conn = getDBConnection(req);
            SBFIFATdPageQuiryAssetDeprecationDAO srvAssetCategoryDAO = new SBFIFATdPageQuiryAssetDeprecationDAO(user, dtoParameter, conn);
            ...
            if (action.equals("")) {
                forwardURL = SrvURLDefineList.PAGE_TD_TRANS_ASSET_DEPRECATION_PAGE;
            } else if (action.equals(SrvWebActionConstant.QUERY_ACTION)) {
                SBFIFATdPageQuiryAssetDeprecationSrv service = new SBFIFATdPageQuiryAssetDeprecationSrv();
                ...
                logDTO.setSynMsg("同步同步MIS资产折旧信息开始");
                ...
                if (srm.getErrorFlag().equalsIgnoreCase("Y")) {
                    SynUpdateDateUtils.createLastUpdateDate(SrvType.SRV_TD_FA_PAGE_ASSET_DEPERACTION, conn);
                    DTOSet ds = service.getDs();
                    if (ds.getSize() > 0) {
                    ...
                    logDTO.setSynType(SrvType.SRV_TD_FA_PAGE_ASSET_DEPERACTION);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步MIS资产折旧信息结束！");
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_TD_FA_PAGE_ASSET_DEPERACTION, conn);
