        ...
        try {
            ...
            if (action.equals("")) {
                ...
                if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    logDTO.setSynMsg("同步TD资产分配行信息(ODI)成功。耗时" + resumeTime + "毫秒，资产账簿：" + dtoParameter.getBookTypeCode());
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_TD_FA_DISTRIBUTION, conn);
                    message.setMessageValue("同步TD资产分配行信息(ODI)成功，" + "耗时" + resumeTime + "毫秒");
                } else {
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_TD_FA_DISTRIBUTION);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步TD资产分配行信息(ODI)失败。耗时" + resumeTime + "毫秒，资产账簿：" + dtoParameter.getBookTypeCode()+"，错误信息:"+srvMessage.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    message.setMessageValue("同步TD资产分配行信息(ODI)失败，" + "耗时" + resumeTime + "毫秒，资产账簿：" + dtoParameter.getBookTypeCode()+"，错误信息:"+srvMessage.getErrorMessage());
                }
                forwardURL = SrvURLDefineList.TD_TRANS_ASSET_DISTRIBUTION_PAGE;
            }
