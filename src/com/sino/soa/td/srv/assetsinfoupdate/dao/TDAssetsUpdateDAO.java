...
import com.sino.soa.td.srv.assetsinfoupdate.model.TDSynAttributeModel;
...
import com.sino.soa.td.srv.assetsinfoupdate.srv.TDUpdateAssetRetirmentSrv;
 ...
 */
public class TDAssetsUpdateDAO extends AMSBaseDAO {
     ...
     */
    public TDAssetsUpdateDAO(SfUserDTO userAccount, TDSrvEamSyschronizeDTO dtoParameter, Connection conn) {
    ...
    protected void initSQLProducer(BaseUserDTO userAccount, DTO dtoParameter) {
        TDSrvEamSyschronizeDTO dtoPara = (TDSrvEamSyschronizeDTO) dtoParameter;
        super.sqlProducer = new TDAssetsUpdateModel((SfUserDTO) userAccount, dtoPara);
    }
        ...
        try {
            if (assetIds != null && assetIds.length > 0) {
                ...
                for (int i = 0; i < assetIds.length; i++) {
                    ...
                    TDSrvAssetsUpdateDAO assetsUpdateDao = new TDSrvAssetsUpdateDAO();
                    TDSrvAssetsUpdateDTO assetsUpdateDto;
    ...
    public String syschronizeAssets(int orgnization_id, String[] assetIds) throws QueryException, SQLException, DataHandleException, CalendarException {
        ...
        try {
            ...
            if (emDto == null) {
                ...
                if (assetIds != null && assetIds.length > 0) {
                    String targetStr = "";
                    TDUpdateAssetRetirmentSrv updateAssetSrv = new TDUpdateAssetRetirmentSrv();
                    for (int i = 0; i < assetIds.length; i++) {
...
//                    batchDTO.setTransStatus(2);
                        batchDTO.setTransStatus(1);//唐明胜修改，1标示同步成功
                        batchDTO.setOrganizationId(orgId);
                        batchDTO.setCreatedBy(userAccount.getUserId());
                        batchDTO.setErrmsg("");
                        batchDTO.setRemark("本次共同步'TD资产基本信息'" + totalCount + "条记录，全部同步成功");
                        synLogUtil.createMisUpdateBatch(batchDTO, conn);

                        totalTagNumbers.clear();//唐明胜增加
                    } else {
                        batchDTO = new EtsMisfaUpdateBatchDTO();
                        batchDTO.setBatchId(this.getNextSeq(conn));
                        batchDTO.setTransType("ASSETSINFO");
                        ...
                        batchDTO.setRemark("本次共同步'TD资产基本信息'" + totalCount + "条记录,同步失败。失败原因："+returnMessage.getErrorMessage());
                        synLogUtil.createMisUpdateBatch(batchDTO, conn);
                    }
                    if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
                        String status = "1";
    ...
    public String getNextSeq(Connection conn) throws SQLException {
        ...
        nextSeq = seq.getGUID();
        //seq.getStrNextSeq("ETS_MISFA_UPDATE_BATCH_S");
        return nextSeq;
    }
    ...
    public List getBookTypeCode() throws QueryException {
        TDSrvAssetsUpdateModel assetsUpdateModel = new TDSrvAssetsUpdateModel((SfUserDTO) userAccount, (TDSrvEamSyschronizeDTO) dtoParameter);
     ...
     * 获取支撑网设备类型
     */
    public EtsMisfaTransactionRespDTO getCexType(String cexId) throws QueryException {
        EtsMisfaTransactionRespDTO dto = null;
        TDSynAttributeModel logModel = new TDSynAttributeModel();
     ...
     * 获取业务平台编码,网络层次编码
     */
    public EtsMisfaTransactionRespDTO getOpe(String opeId) throws QueryException {
        EtsMisfaTransactionRespDTO dto = null;
        TDSynAttributeModel logModel = new TDSynAttributeModel();
