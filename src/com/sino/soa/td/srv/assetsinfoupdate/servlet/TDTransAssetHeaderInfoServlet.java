 ...
 * <p>Description:资产头基本信息同步读取类ODI_TD“TransAssetHeaderInfoServlet”</p>
 * <p>Copyright: Copyright (c) 2011</p>
 * <p>Company: 北京思诺博信息技术有限公司</p>
 * @author wangzp
 * @version 1.0
 */
public class TDTransAssetHeaderInfoServlet extends BaseServlet {
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            if (action.equals("")) {
                forwardURL = SrvURLDefineList.TD_SRV_TRANSASSETHEADERINFO;
                ...
                if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    logDTO.setSynMsg("同步TD资产头信息(ODI)成功,耗时" + resumeTime + "毫秒,资产账簿:"+bookTypeCode);
                    logUtil.synLog(logDTO, conn);
                    message.setMessageValue("同步TD资产头信息(ODI)成功，" + "耗时" + resumeTime + "毫秒");
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_TD_FA_HEADERINFO, conn);
                } else {
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_TD_FA_HEADERINFO);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步TD资产头信息(ODI)失败,耗时" + resumeTime + "毫秒,资产账簿:"+bookTypeCode+" ,错误信息："+srvMessage.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    message.setMessageValue("同步TD资产头信息(ODI)失败，" + "耗时" + resumeTime + "毫秒");
                }
                forwardURL = SrvURLDefineList.TD_SRV_TRANSASSETHEADERINFO;
            } else {
