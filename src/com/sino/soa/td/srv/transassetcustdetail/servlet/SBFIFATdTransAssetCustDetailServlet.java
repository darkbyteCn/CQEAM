package com.sino.soa.td.srv.transassetcustdetail.servlet;
...
import com.sino.soa.td.srv.transassetcustdetail.dto.SBFIFATdTransAssetCustDetailDTO;
import com.sino.soa.td.srv.transassetcustdetail.srv.SBFIFATdTransAssetCustDetailSrv;
 ...
 */
public class SBFIFATdTransAssetCustDetailServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            req2DTO.setDTOClassName(SBFIFATdTransAssetCustDetailDTO.class.getName());
            SBFIFATdTransAssetCustDetailDTO dtoParameter = (SBFIFATdTransAssetCustDetailDTO) req2DTO.getDTO(req);
            ...
            if (action.equals("")) {
                forwardURL = SrvURLDefineList.TD_SRV_ASSET_CUST_DETAIL_PAGE;
                ...
                logDTO.setSynMsg("同步TD转资资产清单(ODI)开始");
                logUtil.synLog(logDTO, conn);
                String envCode = SynUpdateDateUtils.getEnvCode("TDTransAssetCustDetailSrv", conn);
                ...
                if (srvMessage.getErrorFlag().equalsIgnoreCase("Y")) {
                    ...
                    logDTO.setSynType(SrvType.SRV_TD_PA_ASSETDETAIL);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步TD转资资产清单(ODI)成功！");
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_TD_PA_ASSETDETAIL, conn);
                    message.setMessageValue("同步TD转资资产清单(ODI)成功！耗时" + resumeTime + "毫秒。");
                } else {
                    resumeTime = System.currentTimeMillis() - start;
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_TD_PA_ASSETDETAIL);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步TD转资资产清单(ODI)失败！耗时" + resumeTime + "毫秒。错误信息："+srvMessage.getErrorMessage());
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_TD_PA_ASSETDETAIL, conn);

                    message.setMessageValue("同步TD转资资产清单(ODI)失败！耗时" + resumeTime + "毫秒。");
                }
                forwardURL = SrvURLDefineList.TD_SRV_ASSET_CUST_DETAIL_PAGE;
