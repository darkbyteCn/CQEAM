...
import com.sino.soa.td.srv.assetretire.dto.TDSrvRetiredAssetDTO;    //资产会计状态  
import com.sino.soa.util.SynLogUtil;
import com.sino.soa.util.SynUpdateDateUtils;
import com.sino.soa.util.dto.SynLogDTO;
import com.sino.soa.td.srv.assetretire.srv.TDTransRetiredAssetDetailSrv;      //TD处理
        ...
        try {
            ...
            if (action.equals("")) {
                ...
                if (srm.getErrorFlag().equals("Y")) {
                    ...
                    logDTO.setSynType(SrvType.SRV_FA_RETIRE);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步报废资产成功！");
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_FA_RETIRE, conn);
                    message = new Message();
                    message.setMessageValue("同步报废资产成功！，耗时" + resumeTime + "毫秒");
                } else {
                    resumeTime = System.currentTimeMillis() - start;
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_FA_RETIRE);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步报废资产失败！");
                    logUtil.synLog(logDTO, conn);
                    message = new Message();
                    message.setMessageValue("同步报废资产失败！，耗时" + resumeTime + "毫秒");
                }
...
//                String envCode = SynUpdateDateUtils.getEnvCode("TransRetiredAssetDetailSrv", conn);
                ...
                if (srm.getErrorFlag().equals("Y")) {
                    ...
                    logDTO.setSynType(SrvType.SRV_FA_RETIRE);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步报废TD资产成功！");
                    logUtil.synLog(logDTO, conn);
                    SynUpdateDateUtils.updateLastUpdateDate(SrvType.SRV_FA_RETIRE, conn);
                    message = new Message();
                    message.setMessageValue("同步报废TD资产成功！，耗时" + resumeTime + "毫秒");
                } else {
                    resumeTime = System.currentTimeMillis() - start;
                    logDTO = new SynLogDTO();
                    logDTO.setSynType(SrvType.SRV_FA_RETIRE);
                    logDTO.setCreatedBy(user.getUserId());
                    logDTO.setSynMsg("同步报废TD资产失败！");
                    logUtil.synLog(logDTO, conn);
                    message = new Message();
                    message.setMessageValue("同步报废TD资产失败！，耗时" + resumeTime + "毫秒");
                }
