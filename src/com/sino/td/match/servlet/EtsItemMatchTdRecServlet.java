...
import com.sino.td.match.dao.EtsItemMatchTdRecDAO;
import com.sino.td.match.model.EtsItemMatchTdRecModel;
...
public class EtsItemMatchTdRecServlet extends BaseServlet {
...
//               String MATCH_MODE_0THER_RET = "5";    撤销设备屏蔽
        ...
        try {
            ...
            EtsItemMatchTdRecDAO EtsItemMatchTdRecDAO = new EtsItemMatchTdRecDAO(user, dtoParameter, conn);
            String matchType = StrUtil.nullToString(dtoParameter.getMatchType());
            ...
            if (action.equals("")) {
                if (matchType.equals(WebAttrConstant.MATHC_MODE_CHANGED_ASSETS_RET)) {      // 撤消资产匹配关系操作
                    forwardURL = URLDefineList.UNYOKE_TD_PAGE + "?matchType=" + matchType;
                } else {
                ...
                if (matchType.equals(WebAttrConstant.MATHC_MODE_CHANGED_ASSETS_RET)) {
                    BaseSQLProducer sqlProducer = new EtsItemMatchTdRecModel(user, dtoParameter);
                    ...
                    forwardURL = URLDefineList.UNYOKE_TD_PAGE + "?matchType=" + matchType + "&unyokeFlag=" + dtoParameter.getUnyokeFlag();
                } else {
                    BaseSQLProducer sqlProducer = new EtsItemMatchTdRecModel(user, dtoParameter);
                    PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
                    pageDAO.produceWebData();
...
//                }
                    ...
                    if(matchType.equals(WebAttrConstant.MATCH_MODE_PRJMTL) || matchType.equals(WebAttrConstant.MATCH_MODE_PRJMTL_RET)){
                		...
                		EtsItemMatchTdRecDAO.getDistributePrj(dtoParameter.getPrjId(), dtoes);
                    }
                    forwardURL = URLDefineList.FINANCE_PROP_SET_SERVLET + "?act="+ WebActionConstant.QUERY_ACTION + "&matchType=" + matchType;
                }
            } else {
...
//            String[] arr = reqParser.getParameterValues("assetId");
            if (exarr != null) {
                ...
                for (int i = 0; i < exLength; i++) {
                    EtsItemMatchRecDTO dto = new EtsItemMatchRecDTO();
...
//                        dto.setMatchType(StrUtil.strToInt(matchType));
...
//                    }
		...
		try {
            ...
            if(systemids != null){
				...
				for(int i = 0; i < checkedCount; i++){
					...
					dto.setAssetId(StrUtil.strToInt(assetIds[i]));
                    dto.setMatchType(matchType);
                    String flag = req.getParameter("unyokeFlag");
                    if (flag.equals("1")) {                   //资产匹配撤销
                            dto.setOldFinanceProp(DictConstant.FIN_PROP_TD);
    ...
    private DTOSet getSystemIds(HttpServletRequest req) throws UploadException {
        ...
        try {
            ...
            String[] exarr = reqParser.getParameterValues("systemId");
            if (exarr != null) {
            	EtsItemMatchRecDTO dto;
                String inarr;
                for (int i = 0; i < exarr.length; i++) {
                    inarr = exarr[i];
                    if(inarr != null && !inarr.equals("")){
                    	dto = new EtsItemMatchRecDTO();
