...
import com.sino.td.match.dao.ManualMatchTdDAO;
import com.sino.td.match.model.ManualMatchTdAMSModel;
 ...
 *          Date: 2007-11-26
 */
public class ManualMatchTdAMS extends BaseServlet {
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		//String forwardURL = "/td/match/manualMatchTdAMS.jsp";
		String forwardURL = URLDefineList.MANUAL_MATCH_TD_AMS;
		...
		try {
			...
			ManualMatchTdDAO matchDAO = new ManualMatchTdDAO(user, dto, conn);
			ServletConfigDTO configDTO = getServletConfig(req);
			if (act.equals(WebActionConstant.QUERY_ACTION)) {
				AMSSQLProducer sqlProducer = new ManualMatchTdAMSModel(user, dto);
			...
			if (matchSuccess) {
				...
				out.println("location.href=\"/servlet/com.sino.td.match.servlet.ManualMatchTdAMS?act=" + WebActionConstant.QUERY_ACTION + "&key=" + dto.getKey().replaceAll("%","%25") + "&maintainUser="+StrUtil.nullToString(dto.getMaintainUser()).replaceAll("%","%25")+ "&workorderObjectName="+dto.getWorkorderObjectName().replaceAll("%","%25")+"&workorderObjectNo="+dto.getWorkorderObjectNo()+"&countyCode="+dto.getCountyCode()+"&responsibilityDept="+dto.getResponsibilityDept().replaceAll("%","%25")+"&itemName="+dto.getItemName().replaceAll("%","%25")+"&itemSpec="+dto.getItemSpec().replaceAll("%","%25") +"\";");
