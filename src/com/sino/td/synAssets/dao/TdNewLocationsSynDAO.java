 ...
 * Function：    TD新增地點同步
 */
public class TdNewLocationsSynDAO extends AMSBaseDAO {

    /**
     * 功能：TD新增地點同步 数据访问层构造函数
     *
     * @param userAccount  SfUserDTO 代表本系统的最终操作用户对象
     * @param dtoParameter EtsItemMatchDTO 本次操作的数据
     * @param conn         Connection 数据库连接，由调用者传入。
     */
    public TdNewLocationsSynDAO(SfUserDTO userAccount, EamSyschronizeDTO dtoParameter, Connection conn) {
    ...
    protected void initSQLProducer(BaseUserDTO userAccount, DTO dtoParameter) {
        ...
        super.sqlProducer = new TdNewLocationsSynModel((SfUserDTO) userAccount, dtoPara);
    }
    ...
    public void syschronizeLocus(String systemId) {
        ...
        String callStr = "{CALL AMS_SYN_PKG_TD.SYN_TD_NEW_PLACE(?, ?, ?, ?)}";
        try {
            ...
            if(cs.getString(4)!=null)
            {
                 ...
                 Logger.logError("{CALL AMS_SYN_PKG_TD.SYN_TD_NEW_PLACE(?, ?, ?, ?)} ERROR:"+cs.getString(4));
    ...
    public File getExportFile() throws DataTransException {
       File file = null;
       TdNewLocationsSynModel modelProducer = (TdNewLocationsSynModel) sqlProducer;
       SQLModel sqlModel = modelProducer.getPageQueryModel();
       String reportTitle = "TD地点变更同步";
