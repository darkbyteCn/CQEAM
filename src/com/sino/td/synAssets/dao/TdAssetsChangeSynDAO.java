 ...
 * @version 1.0
 */
public class TdAssetsChangeSynDAO extends AMSBaseDAO {

    /**
     * 功能：TD资产变动直接同步 数据访问层构造函数
     * @param userAccount  SfUserDTO 代表本系统的最终操作用户对象
     * @param dtoParameter EtsItemMatchDTO 本次操作的数据
     * @param conn         Connection 数据库连接，由调用者传入。
     */
    public TdAssetsChangeSynDAO(SfUserDTO userAccount, EamSyschronizeDTO dtoParameter, Connection conn) {
    ...
    protected void initSQLProducer(BaseUserDTO userAccount, DTO dtoParameter) {
        ...
        super.sqlProducer = new TdAssetsChangeSynModel((SfUserDTO) userAccount, dtoPara);
    }
     ...
     */
    public synchronized void syschronizeAssets(String orgnizationId, String[] assetIds) {
...
//            conn.setAutoCommit(true);
            ...
            String callStr = "{CALL AMS_SYN_PKG_TD.SYN_ASSET_CHANGE_IMMEDIATE(?, ?)}";
            cs = conn.prepareCall(callStr);
            cs.setString(1, orgnizationId);
    ...
    public File getExportFile() throws DataTransException {
        TdAssetsChangeSynModel modelProducer = (TdAssetsChangeSynModel) sqlProducer;
        SQLModel sqlModel = modelProducer.getPageQueryModel();
        String reportTitle = "TD资产变动直接同步";
    ...
    private Map getFieldMap() {
        ...
        fieldMap.put("MIS_MANUFACTURER", "MIS厂商");
        fieldMap.put("EAM_MANUFACTURER", "EAM厂商");

        fieldMap.put("DATE_PLACED_IN_SERVICE", "MIS启用日期");
        fieldMap.put("COST", "资产原值");
        fieldMap.put("MATCH_USER_NAME", "匹配人");
