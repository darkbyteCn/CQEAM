 ...
 * @version 1.0
 */
public class TdAssetsTransHeaderModel extends AMSSQLProducer {
     ...
     */
    public TdAssetsTransHeaderModel(SfUserDTO userAccount, TdAssetsTransHeaderDTO dtoParameter) {
        super(userAccount, dtoParameter);
        this.user = (SfUserDTO) userAccount;
    }

    /**
     * 功能：框架自动生成资产业务头表(EAM)--取代原表 TD_ASSETS_TRANS_HEADER数据插入SQLModel，请根据实际需要修改。
    ...
    /**
     * 功能：框架自动生成资产业务头表(EAM)--取代原表 TD_ASSETS_TRANS_HEADER数据更新SQLModel，请根据实际需要修改。
    ...
    public SQLModel getDataUpdateModel() throws SQLModelException {
        ...
        try {
            List sqlArgs = new ArrayList();
            TdAssetsTransHeaderDTO dto = (TdAssetsTransHeaderDTO) dtoParameter;
            String sqlStr = "UPDATE TD_ASSETS_TRANS_HEADER"
                            + " SET"
                            + " TRANS_NO = ?,"
                            + " TRANS_STATUS = ?,"
                            + " TO_ORGANIZATION_ID = ?,"
    ...
    /**
     * 功能：框架自动生成资产业务头表(EAM)--取代原表 TD_ASSETS_TRANS_HEADER数据删除SQLModel，请根据实际需要修改。
...
//		sqlArgs.add(userAccount.getUserId());
    ...
    public SQLModel getGroupPidModel() {
        ...
        String sqlStr = " SELECT DISTINCT (SG.P_FLOW_ID)\n" +
                        ...
                        "WHERE SU.USER_ID = SUR.USER_ID " +
                             ...
                             " AND SG.P_FLOW_ID IN(2185, 2223)" +    //综合部、网络部
                             " AND SU.USER_ID = ?";
        List sqlArgs = new ArrayList();
        sqlArgs.add(userAccount.getUserId());
        sqlModel.setSqlStr(sqlStr);
    ...
    public SQLModel getQueryBarcodeExcelModel(String excel, TdAssetsTransHeaderDTO dto) {
        SQLModel sqlModel = new SQLModel();
        List sqlArgs = new ArrayList();
    ...
    public SQLModel getQueryBarcodeAllExcelModel(String excel, TdAssetsTransHeaderDTO dto, String returnModel) {
        SQLModel sqlModel = new SQLModel();
        List sqlArgs = new ArrayList();
        String mtlMgrProps = user.getMtlMgrProps();

        String sqlStr = "SELECT AAAV.BARCODE FROM TD_ASSETS_ADDRESS_V AAAV WHERE AAAV.BARCODE IN (" + excel + ") AND AAAV.BARCODE NOT IN (" + returnModel + ")";
        sqlArgs.add(user.getOrganizationId());
        if (!StrUtil.isEmpty(dto.getFromDept())) { //部门限制
            sqlArgs.add(dto.getFromDept());
	    }
