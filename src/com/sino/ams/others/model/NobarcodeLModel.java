...
public class NobarcodeLModel extends BaseSQLProducer {
	...
	/**
	 * 功能：备件事务行表(EAM) AMS_ITEM_TRANS_L 数据库SQL构造层构造函数
	...
	/**
	 * 功能：框架自动生成备件事务行表(EAM) AMS_ITEM_TRANS_L数据更新SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成备件事务行表(EAM) AMS_ITEM_TRANS_L数据删除SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成备件事务行表(EAM) AMS_ITEM_TRANS_L数据详细信息查询SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成备件事务行表(EAM) AMS_ITEM_TRANS_L多条数据信息查询SQLModel，请根据实际需要修改。
	...
	public SQLModel getDataMuxModel(){
		...
		String sqlStr = "SELECT "
			...
			+ " AMS_ITEM_TRANS_L"
			+ " WHERE"
			+ "( " + SyBaseSQLUtil.isNull() + "  OR TRANS_ID LIKE ?)"
			+ "( " + SyBaseSQLUtil.isNull() + "  OR LINE_ID LIKE ?)"
	 ...
	 * 框架自动生成数据备件事务行表(EAM) AMS_ITEM_TRANS_L详细信息查询SQLModel，请根据实际需要修改。
...
//		sqlArgs.add(sfUser.getOrganizationId());
	 ...
	 * 框架自动生成数据备件事务行表(EAM) AMS_ITEM_TRANS_L 数据删除SQLModel，请根据实际需要修改。
	 * @param transId String
	 * @return SQLModel 返回数据详细信息查询用SQLModel
	 */
	...
	/**
	 * 功能：框架自动生成备件事务行表(EAM) AMS_ITEM_TRANS_L页面翻页查询SQLModel，请根据实际需要修改。
	...
	public SQLModel getPageQueryModel(){
		...
		String sqlStr = "SELECT "
			...
			+ " WHERE"
			+ " ( " + SyBaseSQLUtil.isNull() + "  OR TRANS_ID LIKE ?)"
			+ "AND ( " + SyBaseSQLUtil.isNull() + "  OR LINE_ID LIKE ?)"
