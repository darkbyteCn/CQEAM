...
import com.sino.ams.synchronize.dao.AddressChangedDAO;
 ...
 */
public class AddressChangedServlet extends BaseServlet{
        ...
        public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
            ...
            try {
                ...
                AddressChangedDAO addressChangedDAO = new AddressChangedDAO(user, dto, conn);
                if (action.equals("")) {
                    req.setAttribute(WebAttrConstant.SYSCHRONIZE_DTO, dto);
                    forwardURL = URLDefineList.ADDRESS_CHANGED;
                } else if (action.equals(WebActionConstant.QUERY_ACTION)) { //查询操作
                    BaseSQLProducer sqlProducer = new AddressChangedModel(user, dto);
                    PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
                    pageDAO.setCalPattern(LINE_PATTERN);
                    pageDAO.produceWebData();
                    req.setAttribute(WebAttrConstant.SYSCHRONIZE_DTO, dto);
                    forwardURL = URLDefineList.ADDRESS_CHANGED;
                } else if (action.equals(WebActionConstant.EXPORT_ACTION)) { //导出EXCEL操作
                    File file = addressChangedDAO.exportFile();
