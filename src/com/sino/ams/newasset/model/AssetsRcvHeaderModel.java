...
public class AssetsRcvHeaderModel extends AmsAssetsTransHeaderModel {

	/**
	 * 功能：资产业务头表(EAM) AMS_ASSETS_TRANS_HEADER 数据库SQL构造层构造函数
	 * @param userAccount  SfUserDTO 代表本系统的最终操作用户对象
	 * @param dtoParameter AmsAssetsTransHeaderDTO 本次操作的数据
	 */
	public AssetsRcvHeaderModel(SfUserDTO userAccount, AmsAssetsTransHeaderDTO dtoParameter) {
	...
	public SQLModel getAssetsAssignModel() {
		...
		String sqlStr = "UPDATE"
						...
						+ " NULL"
						+ " FROM"
						+ " AMS_TRANS_ASSIGN_V ATAV"
						+ " WHERE"
						+ " TRANS_ID = ATAV.TRANS_ID"
	...
	public SQLModel getCanReceiveModel() throws SQLModelException {
		SQLModel sqlModel = new SQLModel();
		try {
			AmsAssetsTransHeaderDTO dto = (AmsAssetsTransHeaderDTO)
										  dtoParameter;
			List sqlArgs = new ArrayList();
			String sqlStr = "SELECT"
...
//							+ " AND AATH.FROM_ORGANIZATION_ID = SU.ORGANIZATION_ID"
							...
							+ " AND AATH.FROM_ORGANIZATION_ID = ISNULL(?, AATH.FROM_ORGANIZATION_ID)"
