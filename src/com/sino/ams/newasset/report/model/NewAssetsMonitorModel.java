...
public class NewAssetsMonitorModel extends AMSSQLProducer {

	public NewAssetsMonitorModel(SfUserDTO userAccount, AmsAssetsCheckHeaderDTO dtoParameter) {
    ...
    public SQLModel getPageQueryModel() throws SQLModelException {
		...
		try {
            String sqlStr = "SELECT TOTAL_COUNT.ORGANIZATION_ID,\n" +
                    ...
                    "       ISNULL(SCANCER_IDENTICAL_V.IDENTICAL_COUNT, 0) SCANCER_IDENTICAL_COUNT, --实际扫描完成情况--账实相符数量\n" +
                    "       (CASE CHARINDEX(ROUND(100 * ISNULL(SCANCER_IDENTICAL_V.IDENTICAL_COUNT, 0) /\n" +
                    ...
                    "                ISNULL(SCANED_TOTAL_COUNT.SCANED_COUNT, 1),\n" +
                    "                2) || '%'\n" +
                    "       END) IDENTICAL_RATE, --实际扫描完成情况--账实相符百分比 \n" +
                    "       \n" +
                    "       ISNULL(SCANED_FILTER.SCANED_COUNT, 0) SCANED_FILTER_COUNT, --其中新转资资产扫描数量\n" +
                    "       ISNULL(SCANER_NEED_SCANED_COUNT.SCANED_COUNT, 0) SCANER_NEED_SCANED_COUNT, --已完成新转资资产并且需PDA扫描确认的数量\n" +
                    "       ISNULL(TOTAL_NEED_SCANCEL_COUNT.TOTAL_COUNT, 0) -\n" +
                    "       ISNULL(SCANER_NEED_SCANED_COUNT.SCANED_COUNT, 0) NOT_SCANED_COUNT, --新转资资产尚未扫描确认数量\n" +
                    "       \n" +
                    ...
                    "                   AND AICM.NEW_ASSET_NEED_SCAN = 'Y')\n" +
                    "           AND EOCM.ORGANIZATION_ID = ISNULL(?, EOCM.ORGANIZATION_ID)\n" +
