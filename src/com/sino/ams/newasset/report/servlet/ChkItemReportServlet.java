...
import com.sino.ams.newasset.report.dao.ChkItemReportDAO;
 ...
 * @version 1.0
 */
public class ChkItemReportServlet extends BaseServlet {
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws	ServletException, IOException {
		...
		try {
			...
			if(action.equals("")){
				...
				forwardURL = AssetsURLList.COMP_ITEM_RPT;
			} else if (action.equals(AssetsActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new ChkItemReportModel(user, dto);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
				pageDAO.produceWebData();
				req.setAttribute(QueryConstant.QUERY_DTO, dto);
				forwardURL = AssetsURLList.COMP_ITEM_RPT;
			} else if (action.equals(WebActionConstant.EXPORT_ACTION)) {
				ChkItemReportDAO rptDAO = new ChkItemReportDAO(user, dto, conn);
