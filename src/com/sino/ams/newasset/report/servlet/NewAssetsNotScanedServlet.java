...
import com.sino.ams.newasset.report.dao.NewAssetsNotScanedDAO;
...
public class NewAssetsNotScanedServlet extends BaseServlet {
	/**
	 * 所有的Servlet都必须实现的方法。
	 *
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
            ...
            if (action.equals(AssetsActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new NewAssetsNotScanedModel(user, dto);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
				pageDAO.setPageSize(23);
				pageDAO.setCalPattern(LINE_PATTERN);
				pageDAO.produceWebData();
				req.setAttribute(QueryConstant.QUERY_DTO, dto);
				forwardURL = AssetsURLList.NEW_FA_NOT_SCANED;
			} else if (action.equals(AssetsActionConstant.EXPORT_ACTION)) {
				NewAssetsNotScanedDAO rptDAO = new NewAssetsNotScanedDAO(user, dto, conn);
