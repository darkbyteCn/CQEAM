...
import com.sino.ams.newasset.report.dao.NewAssetsMonitorDAO;
...
public class NewAssetsMonitorServlet extends BaseServlet {
	/**
	 * 所有的Servlet都必须实现的方法。
	 *
...
//			String opt = optProducer.getAllOrganization(dto.getOrganizationId());
			...
			if (action.equals("")) {
				...
				forwardURL = AssetsURLList.NEW_FA_ASSETS_MONITOR_REPORT;
				...
				BaseSQLProducer sqlProducer = new NewAssetsMonitorModel(user, dto);
                ...
                calendar.add(Calendar.DATE, Integer.parseInt(dto.getThreshold()));
                dto.setDeadlineDate(df.format(calendar.getTime()));
				NewAssetsMonitorDAO rptDAO = new NewAssetsMonitorDAO(user, dto, conn);
