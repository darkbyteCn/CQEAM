...
import com.sino.ams.newasset.report.dao.NewAssetsMonitorNotScanedDAO;
...
public class NewAssetsMonitorNotScanedServlet extends BaseServlet {
	/**
	 * 所有的Servlet都必须实现的方法。
	 *
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
            ...
            if (action.equals(AssetsActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new NewAssetsMonitorNotScanedModel(user, dto);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
				pageDAO.setPageSize(30);
				pageDAO.setCalPattern(LINE_PATTERN);
				pageDAO.produceWebData();
				req.setAttribute(QueryConstant.QUERY_DTO, dto);
				forwardURL = AssetsURLList.NEW_FA_ASSETS_MONITOR_NOT_SCANED;
			} else if (action.equals(AssetsActionConstant.EXPORT_ACTION)) {
				NewAssetsMonitorNotScanedDAO rptDAO = new NewAssetsMonitorNotScanedDAO(user, dto, conn);
