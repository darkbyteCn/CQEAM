...
import com.sino.ams.newasset.report.dao.NewAssetsMonitorScanedDAO;
import com.sino.ams.newasset.report.model.NewAssetsMonitorScanedModel;
...
public class NewAssetsMonitorScanedServlet extends BaseServlet {
	/**
	 * 所有的Servlet都必须实现的方法。
	 *
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
            ...
            if (action.equals(AssetsActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new NewAssetsMonitorScanedModel(user, dto);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
				pageDAO.setPageSize(23);
				pageDAO.setCalPattern(LINE_PATTERN);
				pageDAO.produceWebData();
				req.setAttribute(QueryConstant.QUERY_DTO, dto);
				forwardURL = AssetsURLList.NEW_FA_ASSETS_MONITOR_SCANED;
			} else if (action.equals(AssetsActionConstant.EXPORT_ACTION)) {
				NewAssetsMonitorScanedDAO rptDAO = new NewAssetsMonitorScanedDAO(user, dto, conn);
