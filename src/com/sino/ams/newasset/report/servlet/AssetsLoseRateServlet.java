...
import com.sino.ams.newasset.report.dao.AssetsLoseRateDAO;
 ...
 */
public class AssetsLoseRateServlet extends BaseServlet {
	/**
	 * 所有的Servlet都必须实现的方法。
	 *
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
            ...
            String cityOption = "";
            if ( ( user.isProvAssetsManager() && dto.getLoseAssetsType().equals("") ) || user.isSysAdmin() ) {
            ...
            String deptOpt = optProducer.getAsssetsDeptOption(dto.getCountyCode());
            req.setAttribute(AssetsWebAttributes.DEPT_OPTIONS, deptOpt);
            if (action.equals("")) {
				req.setAttribute(QueryConstant.QUERY_DTO, dto);
				forwardURL = AssetsURLList.LOSE_RATE_REPORT;
			} else if (action.equals(AssetsActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new AssetsLoseRateModel(user, dto);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
                pageDAO.setCalPattern(LINE_PATTERN);
                if ( dto.getLoseAssetsType().equals("LOSE_DEPT") && dto.getOrganizationId() < 1) {
				...
				AssetsLoseRateDAO rptDAO = new AssetsLoseRateDAO(user, dto, conn);
