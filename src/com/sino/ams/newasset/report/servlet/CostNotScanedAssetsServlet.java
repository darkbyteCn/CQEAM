...
import com.sino.ams.newasset.report.model.CostNotScanedAssetsModel;
...
public class CostNotScanedAssetsServlet extends BaseServlet {
	/**
	 * 所有的Servlet都必须实现的方法。
	 *
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (action.equals(WebActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new CostNotScanedAssetsModel(user, dto);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
				pageDAO.setPageSize(23);
                pageDAO.setCalPattern(LINE_PATTERN);
                pageDAO.produceWebData();
				forwardURL = AssetsURLList.COST_NOT_SCANED_ASSETS;
			} else {
