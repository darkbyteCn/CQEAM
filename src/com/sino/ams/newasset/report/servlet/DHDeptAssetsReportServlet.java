...
import com.sino.ams.newasset.report.dao.DHDeptAssetsReportDAO;
...
public class DHDeptAssetsReportServlet extends BaseServlet {
	/**
	 * 所有的Servlet都必须实现的方法。
	 *
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
            ...
            if (action.equals("")) {
				...
				forwardURL = AssetsURLList.DH_DEPT_ASSETS_REPORT;
			} else if (action.equals(AssetsActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new DHDeptAssetsReportModel(user, dto);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
				pageDAO.produceWebData();
                req.setAttribute(QueryConstant.QUERY_DTO, dto);
				forwardURL = AssetsURLList.DH_DEPT_ASSETS_REPORT;
			} else if (action.equals(AssetsActionConstant.EXPORT_ACTION)) {
				DHDeptAssetsReportDAO rptDAO = new DHDeptAssetsReportDAO(user, dto, conn);
