...
public class OrderQueryServlet extends BaseServlet {
    ...
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws
        ...
        try {
            ...
            if (transType.equals(AssetsDictConstant.ASS_RED) || transType.equals(AssetsDictConstant.ASS_RFU)) {
            ...
            if (action.equals("")) {
                ...
                forwardURL = AssetsURLList.ORDER_QUERY_PAGE;
                ...
                if (dto.getTransType().equals("ASS-RFU")) {
                	dto.setTransType("ASS-RED"); //以适应调拨查询的页面
                }
                req.setAttribute(QueryConstant.QUERY_DTO, dto);
                forwardURL = AssetsURLList.ORDER_QUERY_PAGE;
    ...
    public File exportFile(SfUserDTO user, AmsAssetsTransHeaderDTO dto, Connection conn) throws DataTransException {
        ...
        try {
            ...
            if (transType.equals(AssetsDictConstant.ASS_RED)) {
                ...
                fieldMap.put("FROM_DEPT_NAME", "调出单位");
                fieldMap.put("CREATED", "调拨申请人");
                fieldMap.put("CREATION_DATE", "调拨申请日期");
                fieldMap.put("TRANS_STATUS_DESC", "单据状态");
            } else if (transType.equals(AssetsDictConstant.ASS_FREE)) {
            	fieldMap.put("TRANS_NO", "闲置单号");
                fieldMap.put("TRANS_STATUS_DESC", "单据状态");
                fieldMap.put("FROM_DEPT_NAME", "闲置部门");
                fieldMap.put("CREATED", "申请人");
                fieldMap.put("CREATION_DATE", "创建日期");
			} else {
                fieldMap.put("TRANS_NO", "报废单号");
                fieldMap.put("TRANS_STATUS_DESC", "单据状态");
                fieldMap.put("FROM_DEPT_NAME", "报废部门");
                fieldMap.put("CREATED", "申请人");
                fieldMap.put("CREATION_DATE", "创建日期");
            }
