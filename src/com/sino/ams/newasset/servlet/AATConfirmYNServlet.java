...
import com.sino.ams.newasset.dao.AATConfirmYNDAO;
 ...
 * <p>Copyright: 北京思诺博信息技术有限公司版权所有</p>
 * <p>Company: 北京思诺博信息技术有限公司</p>
 */
public class AATConfirmYNServlet extends BaseServlet {
	/**
	 * 所有的Servlet都必须实现的方法。
	 *
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws
		...
		try {
			...
			String confirmYN = req.getParameter("confirmYN") == null ? "" : req.getParameter("confirmYN");
			dto.setCalPattern(LINE_PATTERN);
			conn = getDBConnection(req);
			AATConfirmYNDAO confirmDAO = new AATConfirmYNDAO(user, dto, conn);
			if (action.equals("")) {
				dto.setCalPattern(LINE_PATTERN);
				req.setAttribute(QueryConstant.QUERY_DTO, dto);
				forwardURL = AssetsURLList.AAT_CONFIRM_YN_PAGE;
			} else if (action.equals(AssetsActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new AATConfirmYNModel(user, dto, confirmYN);
				...
				dto.setCalPattern(LINE_PATTERN);
				req.setAttribute(QueryConstant.QUERY_DTO, dto);
				req.setAttribute("confirmYN", confirmYN);
				forwardURL = AssetsURLList.AAT_CONFIRM_YN_PAGE;
				...
				forwardURL = AssetsURLList.AAT_CONFIRM_YN_SERVLET;
				...
				if (exportType.equals(AssetsWebAttributes.EXPORT_SELECTED_ASSETS)) {
					...
					file = confirmDAO.exportQueryAssets(confirmYN);
				}
