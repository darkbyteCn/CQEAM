	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws
        ...
        try {
            ...
            if (action.equals("")) {
                ...
                int yczCount = itemVillageDAO.checkItemStatus(barcodes, "DISCARDED_YCZ");//报废已处置
                int yubfCount = itemVillageDAO.checkItemStatus(barcodes, "PRE_DISCARDE");//预报废
                int zhCount = itemVillageDAO.checkItemStatus(barcodes, "EXCHANGE");//置换
                int yzzCount = itemVillageDAO.checkItemStatus(barcodes, "PRE_ASSETS");//预转资
                int xsCount = itemVillageDAO.checkItemStatus(barcodes, "SELL");//销售
                int czCount = itemVillageDAO.checkItemStatus(barcodes, "RENT");//出租
                int zyCount = itemVillageDAO.checkItemStatus(barcodes, "NORMAL");//在用
                int ztCount = itemVillageDAO.checkItemStatus(barcodes, "ON_WAY");//在途
                int sxCount = itemVillageDAO.checkItemStatus(barcodes, "SEND_REPAIR");//送修
                int dxCount = itemVillageDAO.checkItemStatus(barcodes, "TO_REPAIR");//待修
                int dbfCount = itemVillageDAO.checkItemStatus(barcodes, "TO_DISCARD");//待报废
                int yibfCount = itemVillageDAO.checkItemStatus(barcodes, "DISCARDED");//已报废
                int yczfCount = itemVillageDAO.checkItemStatus(barcodes, "CLEARED");//已处置
                int xzCount = itemVillageDAO.checkItemStatus(barcodes, "FREE");//闲置
                int shCount = itemVillageDAO.checkItemStatus(barcodes, "DAMAGED");//损坏
                int yibfGsjCount = itemVillageDAO.checkItemStatus(barcodes, "DISCARDED_TRANS");//已报废-公司间调拨
                int jzCount = itemVillageDAO.checkItemStatus(barcodes, "DONATE");//捐赠
                int dbfGsjCount = itemVillageDAO.checkItemStatus(barcodes, "TO_DISCARD_TRANS");//待报废-公司间调拨
