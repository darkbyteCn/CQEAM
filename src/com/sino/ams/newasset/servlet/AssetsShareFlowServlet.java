...
import com.sino.ams.freeflow.OrderApproveDAO;
...
public class AssetsShareFlowServlet extends BaseServlet {
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        String forwardURL = "";
 		...
 		try {
             ...
             if (action.equals("")) {//进入创建共享申请的可查询页面
 				req.setAttribute(QueryConstant.QUERY_DTO, dto);
 				forwardURL = URLDefineList.ASSETS_SHARE_FLOW_PAGE;
 			} else if (action.equals(AssetsActionConstant.QUERY_ACTION)) {//在创建共享申请栏目下查询 共享单据 为暂存状态
 				dto.setTransStatus(AssetsDictConstant.SAVE_TEMP);
 				BaseSQLProducer sqlProducer = new AssetsShareFlowModel(user, dto);
 			...
 			} else if (action.equals(AssetsActionConstant.NEW_ACTION)) {//在创建共享申请栏目下，点击“新增”进入新增共享申请的页面
 				...
 				headerDTO.setCalPattern(LINE_PATTERN);
 				 if(provinceCode.equals(AssetsDictConstant.PROVINCE_CODE_SN)) {
 					...
 					forwardURL = URLDefineList.ASSETS_SHARE_FLOW_EDIT_PAGE;
 				}
				...
				message.addParameterValue("共享单： " + dto.getTransNo());
 				forwardURL = "/servlet/com.sino.ams.newasset.servlet.AssetsShareFlowServlet";
 				if (transId.equals("")) {
 					forwardURL += "?act=" + AssetsActionConstant.NEW_ACTION;
 				} else {
 			...
 			} else if (action.equals(AssetsActionConstant.CANCEL_ACTION)) { //撤销暂存的共享申请
 				CheckBoxProp checkProp = new CheckBoxProp("subCheck");
 				checkProp.addDbField("TRANS_ID");
                 ...
                 AssetsShareFlowModel freeFlowModel = new AssetsShareFlowModel(user, dto);
...
//                 req.setAttribute(WebAttrConstant.WORKORDER_LOC_ROWSET, rowSet);
	...
	private FreeFlowDTO fillOptions(FreeFlowDTO dto, SfUserDTO user, Connection conn) throws
		...
		String transTypeValue = flexUtil.getFlexValue(AssetsDictConstant.ORDER_TYPE_ASSETS, dto.getTransType());
		dto.setTransTypeValue(transTypeValue);
		AssetsOptProducer optProducer = new AssetsOptProducer(user, conn);
