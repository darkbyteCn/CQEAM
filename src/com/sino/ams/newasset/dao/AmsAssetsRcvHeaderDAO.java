...
public class AmsAssetsRcvHeaderDAO extends AMSBaseDAO {
	 ...
	 */
	public AmsAssetsRcvHeaderDAO(SfUserDTO userAccount, AmsAssetsRcvHeaderDTO dtoParameter, Connection conn) {
	...
	protected void initSQLProducer(BaseUserDTO userAccount, DTO dtoParameter) {
		...
		sqlProducer = new AmsAssetsRcvHeaderModel((SfUserDTO) userAccount,dto);
	}

	/**
	 * 功能：根据调拨单头信息和接收分配行信息自动生成接收审批单
	 * @param order AmsAssetsTransHeaderDTO
	 * @param assignLines DTOSet
	 * @throws DataHandleException
	 */
	public void autoCreateRcvDatas(AmsAssetsTransHeaderDTO order, DTOSet assignLines) throws DataHandleException {
		createRcvOrder(order);
		AmsAssetsRcvLineDAO rcvLineDAO = new AmsAssetsRcvLineDAO(userAccount, null, conn);
		AmsAssetsRcvHeaderDTO dto = (AmsAssetsRcvHeaderDTO) dtoParameter;
	 ...
	 * @throws DataHandleException
	 */
	private void createRcvOrder(AmsAssetsTransHeaderDTO order) throws
			DataHandleException {
		try {
			AmsAssetsRcvHeaderDTO dto = new AmsAssetsRcvHeaderDTO();
			SeqProducer seqProducer = new SeqProducer(conn);
//			String rcvId = seqProducer.getStrNextSeq("AMS_ASSETS_RCV_HEADER_S");
			String rcvId = seqProducer.getGUID();
			...
			dto.setTransId(StrUtil.strToInt(order.getTransId()));
			dto.setReceiveUser(userAccount.getUserId());
			dto.setOrderStatus(AssetsDictConstant.SAVE_TEMP);
			dto.setReceiveOrganizationId(userAccount.getOrganizationId());
			...
			setDTOParameter(dto);
			AmsAssetsRcvHeaderModel modelProducer = (AmsAssetsRcvHeaderModel) sqlProducer;
	...
	public boolean canApproveCurrTask() {
		...
		try {
			AmsAssetsRcvHeaderModel modelProducer = (AmsAssetsRcvHeaderModel)
					sqlProducer;
