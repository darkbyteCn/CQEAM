...
import com.sino.ams.newasset.model.AssetsRcvLineModel;
...
public class AssetsLineReceiveDAO extends AMSBaseDAO {
	 ...
	 */
	public AssetsLineReceiveDAO(SfUserDTO userAccount, AmsAssetsTransLineDTO dtoParameter, Connection conn) {
	...
	protected void initSQLProducer(BaseUserDTO userAccount, DTO dtoParameter) {
		...
		sqlProducer = new AssetsRcvLineModel(user, dto);
	}

	public void setOrderHeader(AmsAssetsTransHeaderDTO headerDTO) {
	...
	public void rcvAssetLines(DTOSet assetLines) throws DataHandleException {
		AssetsRcvLineModel modelProducer = new AssetsRcvLineModel(userAccount, null);
		SQLModel sqlModel = null;
		for (int i = 0; i < assetLines.getSize(); i++) { //分配资产
			AmsAssetsTransLineDTO dto = (AmsAssetsTransLineDTO) assetLines.
										getDTO(i);
		...
		try {
			...
			for (int i = 0; i < assetLines.getSize(); i++) { //分配资产
				try {
					AmsAssetsTransLineDTO dto = (AmsAssetsTransLineDTO)
												assetLines.getDTO(i);
					saveMessage(dto);
	 ...
	 * @throws DataHandleException
	 */
	private void saveMessage(AmsAssetsTransLineDTO dto) throws
			DataHandleException {
		try {
			...
			if (sq.hasResult()) {
				...
				msgDefineDTO.setApplyNumber(StrUtil.nullToString(dto.getLineId()));
				msgDefineDTO.setMsgContent(dto.getResponsibilityUserName() +
										   "您有新的资产需要确认。");
//				msgSaver.saveMsg(msgDefineDTO); （短信，暂时注释掉）
			}
