 ...
 *          Date: 2007-11-28
 */
public class ManualMatchDAO extends AMSBaseDAO {
	private int matchedItemCount = 0; //本次匹配成功的数量

	public ManualMatchDAO(BaseUserDTO userAccount, DTO dtoParameter,
			Connection conn) {
		...
		if ("Y".equalsIgnoreCase(userAccount.getIsTd())) {
            String sqlStr="";
             if ("Y".equalsIgnoreCase(userAccount.getIsTt())) {
                   sqlStr = "{call dbo.EIMP_SAVE_ITEM_MATCH_TT(?,?,?,?,?)}";
             }else{
                  sqlStr = "{call dbo.EIMP_SAVE_ITEM_MATCH_TD(?,?,?,?,?)}";
             }
			...
			try {
				...
				for (int i = 0; i < systemIds.length; i++) {
					cStmt.setString(1, systemIds[i].toString());
					cStmt.setInt(2, StrUtil.strToInt(assetId));
           ...
           String sqlStr="";
			...
			String sqlStr = "{call dbo.EIMP_SAVE_ITEM_MATCH(?,?,?,?,?)}";
			try {
				autoCommit = conn.getAutoCommit();
				conn.setAutoCommit(false);
				cStmt = conn.prepareCall(sqlStr);
	...
	public synchronized boolean matchByType(String systemid, String assetId,
		...
		boolean success = false;
		String sqlStr = "";
		if ("Y".equalsIgnoreCase(userAccount.getIsTd())) {  //TD没有批量匹配，此段代码暂时不会被执行。
			switch (type) {
			case 1:
