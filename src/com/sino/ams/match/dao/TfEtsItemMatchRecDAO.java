...
public class TfEtsItemMatchRecDAO extends BaseDAO {
     ...
     */
    public TfEtsItemMatchRecDAO(SfUserDTO userAccount, TfEtsItemMatchRecDTO dtoParameter, Connection conn) {
    ...
    protected void initSQLProducer(BaseUserDTO userAccount, DTO dtoParameter) {
    	TfEtsItemMatchRecDTO dtoPara = (TfEtsItemMatchRecDTO) dtoParameter;
        super.sqlProducer = new TfEtsItemMatchRecModel((SfUserDTO) userAccount, dtoPara);
    }
    ...
    public void saveDTOs(DTOSet dtos) throws DataHandleException {
        if (dtos != null && dtos.getSize() > 0) {
            ...
            for (int i = 0; i < dtoCount; i++) {
            	TfEtsItemMatchRecDTO dto = (TfEtsItemMatchRecDTO) dtos.getDTO(i);
                saveDTO(dto);
            }
        }
    }

    public void saveDTO(TfEtsItemMatchRecDTO dto) throws DataHandleException {
        boolean hasError = true;
        boolean autoCommit = false;
        try {
            autoCommit = conn.getAutoCommit();
            conn.setAutoCommit(false);
            TfEtsItemMatchRecModel modelProducer = (TfEtsItemMatchRecModel) sqlProducer;
    ...
    public void insertIntoEIMR(TfEtsItemMatchRecDTO dto) throws DataHandleException {
    	TfEtsItemMatchRecModel modelProducer = (TfEtsItemMatchRecModel) sqlProducer;
    ...
    public void updateEIMR(TfEtsItemMatchRecDTO dto) throws DataHandleException {
    	TfEtsItemMatchRecModel modelProducer = (TfEtsItemMatchRecModel) sqlProducer;
    ...
    //插入ETS_ITEM_MATCH_REC_LOG
    public void insertIntoEIMRL(TfEtsItemMatchRecDTO dto) throws DataHandleException {
    	TfEtsItemMatchRecModel modelProducer = (TfEtsItemMatchRecModel) sqlProducer;
...
//               7、	更改FA_ADDITIONS_B@MIS_FA相关信息
    public void delDTO(TfEtsItemMatchRecDTO dto) throws DataHandleException {
        boolean hasError = true;
        boolean autoCommit = false;
        String tagNumber = "";
        String misTagNumber = "";
        try {
           ...
           //更新ETS_ITEM_MATCH_TF_COMPLET
            sqlModel = modelProducer.updateeimcModel(dto);
            DBOperator.updateRecord(sqlModel, conn);

            //删除ets_item_match_tf表的相应记录
            sqlModel = modelProducer.delFromMatchTbl(dto);
            DBOperator.updateRecord(sqlModel, conn);

            //查询ETS_ITEM_MATCH_TF_REC，是否存在该设备的记录
            sqlModel = modelProducer.getHasBeenModel(dto);
            SimpleQuery sq = new SimpleQuery(sqlModel, conn);
            sq.executeQuery();
            //如果没有插入,如果有更新.
            ...
            //插入ETS_ITEM_MATCH_TF_REC_LOG
            insertIntoEIMRL(dto);
            //更新ETS_FA_ASSETS_TF信息
