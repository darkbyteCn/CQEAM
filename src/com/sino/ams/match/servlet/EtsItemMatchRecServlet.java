...
import com.sino.ams.match.dao.EtsItemMatchRecDAO;
...
public class EtsItemMatchRecServlet extends BaseServlet {
...
//               String MATCH_MODE_0THER_RET = "5";    撤销设备屏蔽
        ...
        try {
            ...
            EtsItemMatchRecDAO etsItemMatchRecDAO = new EtsItemMatchRecDAO(user, dtoParameter, conn);
            ...
            if (action.equals("")) {
                if (matchType.equals(WebAttrConstant.MATHC_MODE_CHANGED_ASSETS_RET)) {      // 撤消资产匹配关系操作
                    forwardURL = URLDefineList.UNYOKE_PAGE + "?matchType=" + matchType;
                } else {
                ...
                if (matchType.equals(WebAttrConstant.MATHC_MODE_CHANGED_ASSETS_RET)) {
                    BaseSQLProducer sqlProducer = new EtsItemMatchRecModel(user, dtoParameter);
                    ...
                    forwardURL = URLDefineList.UNYOKE_PAGE + "?matchType=" + matchType + "&unyokeFlag=" + dtoParameter.getUnyokeFlag();
                } else {
                    BaseSQLProducer sqlProducer = new EtsItemMatchRecModel(user, dtoParameter);
                    PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
                    pageDAO.produceWebData();
		...
		try {
            ...
            if(systemids != null){
				...
				for(int i = 0; i < checkedCount; i++){
                    ...
                    dto.setMatchType(matchType);
                    String flag = StrUtil.nullToString(req.getParameter("unyokeFlag"));
                    if (flag.equals("1")) {                   //资产匹配撤销
                            dto.setOldFinanceProp(DictConstant.FIN_PROP_ASSETS);
                            dto.setNewFinanceProp(DictConstant.FIN_PROP_UNKNOW);
                        } else if (flag.equals("0")) {            //转资匹配撤销
