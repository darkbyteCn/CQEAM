...
import com.sino.ams.dzyh.dao.EtsObjectDAO;
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (action.equals("")) {
				...
				forwardURL = URLDefineList.DZYH_QUERY_PAGE;
				...
				req.setAttribute(WebAttrConstant.COUNTY_OPTION, countyOption);
				req.setAttribute(WebAttrConstant.ETS_OBJECT_DTO, dtoParameter);
				forwardURL = URLDefineList.DZYH_DETAIL_PAGE;
				...
				String i = Integer.toString((new Random()).nextInt());
				int code = Integer.parseInt(i.substring(3, 8));
				dtoParameter.setWorkorderObjectCode("INI" + code);
				String countyName = dtoParameter.getCountyName();
				String workorderObjectName = dtoParameter.getWorkorderObjectName();
				String workorderObjectLocation = countyName + "." + workorderObjectName + ".000";
				dtoParameter.setWorkorderObjectLocation(workorderObjectLocation);
				...
				forwardURL = URLDefineList.DZYH_QUERY_SERVLET;
			} else if (action.equals(AMSActionConstant.INURE_ACTION)) {    // 详细页面生效操作 （单个生效）
				etsObjectDAO.inureData();
				String countyOption = optProducer.getCountyOption(dtoParameter.getCountyCode());
				req.setAttribute(WebAttrConstant.COUNTY_OPTION, countyOption);
				String cityOption = optProducer.getAllOrganization(  SyBaseSQLUtil.NULL_INT_VALUE  );
