...
import com.sino.ams.instrument.dao.AmsInstrumentRepairDAO;
import com.sino.ams.instrument.dto.AmsInstrumentHDTO;
import com.sino.ams.instrument.model.AmsInstrumentRepairModel;
 ...
 */
public class AmsInstrumentRepairServlet extends BaseServlet {
	 public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			AmsInstrumentRepairDAO dao = new AmsInstrumentRepairDAO(user, dtoParameter, conn);
			...
			if (action.equals("")) {
				...
				forwardURL = "/instrument/instruRepair.jsp";
			} else if (action.equals(WebActionConstant.QUERY_ACTION)) {
				BaseSQLProducer sqlProducer = new AmsInstrumentRepairModel(user, dtoParameter);
...
//                amsInstrumentInfo.setRepaireDate(CalendarUtil.getCurrCalendar());    //获取当前的日期
				amsInstrumentInfo.setRepaireDate(CalendarUtil.getCurrDate());
				amsInstrumentInfo.setTransType(DictConstant.INS_REP);  //交易类型:送修单据
				req.setAttribute(WebAttrConstant.AMS_INSTRUMENTH_DTO, amsInstrumentInfo);
				forwardURL = "/instrument/instruRepairDetail.jsp";
				...
				if (operateResult) {
					...
					forwardURL = "/servlet/com.sino.ams.instrument.servlet.AmsInstrumentRepairServlet?act=" + WebActionConstant.DETAIL_ACTION
				...
				fileDown.download();
				file.delete();
			 } else if (action.equals(AMSActionConstant.BORROW_ACTION)) {   //送修操作
				Request2DTO r2 = new Request2DTO();
				r2.setDTOClassName(AmsInstrumentHDTO.class.getName());
				r2.setIgnoreFields(AmsInstrumentHDTO.class);
				DTOSet lineSet = r2.getDTOSet(req);
				...
				message.setIsError(!operateResult);
				if (operateResult) {
					showMsg = "仪器送修单" + dtoParameter.getTransNo() + "已生成!";
					forwardURL = "/instrument/instruRepair.jsp";
				} else {
					forwardURL = "/servlet/com.sino.ams.instrument.servlet.AmsInstrumentRepairServlet?act=" + WebActionConstant.DETAIL_ACTION
							+ "&transId=" + dtoParameter.getTransId();
