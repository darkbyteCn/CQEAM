 ...
 */
public class AmsInstrumentRepairDAO extends AMSBaseDAO {
    private AmsInstrumentHDTO dto = null;

    public AmsInstrumentRepairDAO(SfUserDTO userAccount, AmsInstrumentHDTO dtoParameter, Connection conn) {
    ...
    protected void initSQLProducer(BaseUserDTO userAccount, DTO dtoParameter) {
        ...
        super.sqlProducer = new AmsInstrumentRepairModel((SfUserDTO) userAccount, dtoPara);
    }
    ...
    public boolean saveData(DTOSet lineSet, String[] barcode) throws SQLException {
        ...
        try {
            ...
            if (transId.equals("")) {
                ...
                createData();
                AmsInstrumentRepairModel model = new AmsInstrumentRepairModel(userAccount, null);
...
//             AmsInstrumentHDTO itemDTO = (AmsInstrumentHDTO) dtoParameter;
            String transId = dto.getTransId();
            dto.setBorrowDate(CalendarUtil.getCurrDate());         //生成送修单
            OrderNumGenerator ong = new OrderNumGenerator(conn, userAccount.getCompanyCode(), "INS-REP");
            dto.setTransNo(ong.getOrderNum());    //获得单据号
...
//             AmsInstrumentHDTO itemDTO = (AmsInstrumentHDTO) dtoParameter;
            ...
            if (transId.equals("")) {
                ...
                dto.setTransId(transId);
                createData();                   //对仪器仪表头表进行插入操作
                String amintain = StrUtil.nullToString(dto.getBorrowUser());
                AmsInstrumentRepairModel model = new AmsInstrumentRepairModel(userAccount, null);
                for (int i = 0; i < barcode.length; i++) {
                    String code = barcode[i];
                    DBOperator.updateRecord(model.insertLData(code, transId), conn);      //仪器仪表行表
