 ...
 */
public class AmsWorkPlanPigeonholeModel extends BaseSQLProducer {
    private AmsWorkPlanDTO amsWorkPlan = null;
    private SfUserDTO SfUser = null;

    public AmsWorkPlanPigeonholeModel(SfUserDTO userAccount, AmsWorkPlanDTO dtoParameter) {
    ...
    public SQLModel getDataPigeonholeModel(String planId) {
        SQLModel sqlModel = new SQLModel();
        List sqlArgs = new ArrayList();
        String sqlStr = "UPDATE AMS_WORK_PLAN SET PLAN_STATUS = 5 WHERE PLAN_ID = ?";
        sqlArgs.add(planId);
        sqlModel.setSqlStr(sqlStr);
        sqlModel.setArgs(sqlArgs);
        return sqlModel;
