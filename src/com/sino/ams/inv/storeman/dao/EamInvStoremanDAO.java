...
public class EamInvStoremanDAO extends AMSBaseDAO {
	...
	public void saveDatas(String[] storemanIds, String[] workorderObjectNos, String userId)throws DataHandleException{
		...
		try {
			...
			for(int i = 0; i < dataCount; i++){
				...
				if(StrUtil.isEmpty(storemanId)){
					cst.setInt(2, -1);
				} else {
					cst.setInt(2, StrUtil.strToInt(storemanId));
				}
				cst.setInt(3, StrUtil.strToInt(workorderObjectNos[i]));
				cst.setInt(4, StrUtil.strToInt(userId));
				cst.setInt(5, userAccount.getUserId());
	...
	/**
	 * 功能：批量更新仓管员(EAM)表"EAM_INV_STOREMAN"数据。
	 */
	...
	/**
	 * 功能：批量插入仓管员(EAM)表"EAM_INV_STOREMAN"数据。
	 */
