 ...
 *          Date: 2007-11-13
 */
public class BjfkServlet extends BaseServlet {
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (action.equals("")) {
				...
				forwardURL = "/spare/bjfkCreate.jsp";
				...
				if (operateResult) {
					forwardURL = "/servlet/com.sino.ams.spare.servlet.BjfkServlet"
				...
				forwardURL = "/servlet/com.sino.ams.spare.servlet.BjfkServlet"
							 + "?act=" + WebActionConstant.DETAIL_ACTION
							 + "&transId=" + dto.getTransId();
			} else if (action.equals("print")) { //坏件人库单打印
				itemTransHDAO.setDTOClassName(AmsItemTransHDTO.class.getName());
				AmsItemTransHDTO orderHeader = (AmsItemTransHDTO) itemTransHDAO.getDataByPrimaryKey();
				if (orderHeader == null) {
					orderHeader = new AmsItemTransHDTO();
				...
				AmsItemTransLDAO ldao = new AmsItemTransLDAO(user, null, conn);
				req.setAttribute("AIT_LINES", ldao.getLines(orderHeader.getTransId()));
				forwardURL = "/spare/print/spareHJRKPrint.jsp";
