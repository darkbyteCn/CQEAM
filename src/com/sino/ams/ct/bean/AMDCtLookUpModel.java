...
public class AMDCtLookUpModel extends LookUpModel {

    private SfUserDTO user = null;

    public AMDCtLookUpModel(BaseUserDTO userAccount, DTO dtoParameter, LookUpProp lookProp) {
...
//   by xupz 2008-06-20   " AND AMS_PUB_PKG.GET_REPAIRE_DATE(EII.BARCODE,?)  " + SyBaseSQLUtil.isNotNull("SRD.RES_URL") + " " +
                    " AND( " + SyBaseSQLUtil.isNull() + "  OR EII.BARCODE LIKE ?)\n" +
                    " AND( " + SyBaseSQLUtil.isNull() + "  OR ESI.ITEM_NAME LIKE ?)" +
                    " AND( " + SyBaseSQLUtil.isNull() + "  OR ESI.ITEM_SPEC LIKE ?)";
            sqlArgs.add(dto.getVendorName());
...
//                    "       ASI.QUANTITY ONHAND_QTY,\n" +
            ...
            sqlStr = "SELECT AACB.BATCH_NO WORKORDER_BATCH,\n" +
                    ...
                    "       AND ( " + SyBaseSQLUtil.isNull() + "  OR AACB.BATCH_NO LIKE dbo.NVL(?, AACB.BATCH_NO))\n" +
