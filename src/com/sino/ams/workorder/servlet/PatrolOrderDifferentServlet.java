...
import com.sino.ams.workorder.dao.PatrolOrderDifferentDAO;
import com.sino.ams.workorder.dto.EtsWorkorderDTO;
import com.sino.ams.workorder.model.PatrolOrderDifferentModel;
 ...
 * Function:	巡检工单清单差异
 */
public class PatrolOrderDifferentServlet extends BaseServlet {
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (action.equals("")) {
				...
				forwardURL = URLDefineList.PATROL_ORDER_DIFFERENT_QUERY;
			} else if (action.equals(WebActionConstant.QUERY_ACTION)) {
				req.setAttribute(QueryConstant.QUERY_DTO,dtoParameter);
				BaseSQLProducer sqlProducer = new PatrolOrderDifferentModel(user, dtoParameter);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
				pageDAO.produceWebData();
				forwardURL = URLDefineList.PATROL_ORDER_DIFFERENT_QUERY;
			} else if (action.equals(AssetsActionConstant.EXPORT_ACTION)) {
				PatrolOrderDifferentDAO dao = new PatrolOrderDifferentDAO(user, dtoParameter, conn);
