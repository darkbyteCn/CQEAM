...
import com.sino.ams.workorder.dao.TrunListDifferentQueryDAO;
 ...
 * Function:	转资清单差异
 */
public class TrunListDifferentQueryServlet extends BaseServlet {
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			if (action.equals("")) {
				...
				forwardURL = URLDefineList.TRUN_LIST_DIFFERENT_QUERY;
			} else if (action.equals(WebActionConstant.QUERY_ACTION)) {
				req.setAttribute(QueryConstant.QUERY_DTO,dtoParameter);
				BaseSQLProducer sqlProducer = new TrunListDifferentQueryModel(user, dtoParameter);
				PageQueryDAO pageDAO = new PageQueryDAO(req, conn, sqlProducer);
                pageDAO.setCalPattern(LINE_PATTERN);
				pageDAO.produceWebData();
				forwardURL = URLDefineList.TRUN_LIST_DIFFERENT_QUERY;
			} else if (action.equals(AssetsActionConstant.EXPORT_ACTION)) {
				TrunListDifferentQueryDAO dao = new TrunListDifferentQueryDAO(user, dtoParameter, conn);
