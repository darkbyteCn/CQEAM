 ...
 */
public class AssetsSamplingReportModel2 extends AMSSQLProducer {

	public AssetsSamplingReportModel2(SfUserDTO userAccount, AmsAssetsSamplingHeaderDTO dtoParameter) {
	...
	public SQLModel getPageQueryModel() throws SQLModelException {
        ...
        try {
            String sqlStr = "SELECT TOTAL_1.COMPANY,\n" +
                    ...
                    "       (CASE CHARINDEX('.', STR(ROUND(100.0 * ISNULL(TOTAL_2.SCAN_COUNT, 0) / CASE ISNULL(TOTAL_1.REQ_COUNT, 1) WHEN 0 THEN 1 ELSE ISNULL(TOTAL_1.REQ_COUNT, 1) END, 5), 10, 2) || '%')\n" +
                    "       WHEN 1 THEN\n" +
                    "       '0' || STR(ROUND(100.0 * ISNULL(TOTAL_2.SCAN_COUNT, 0) / CASE ISNULL(TOTAL_1.REQ_COUNT, 1) WHEN 0 THEN 1 ELSE ISNULL(TOTAL_1.REQ_COUNT, 1) END, 5), 10, 2) || '%'\n" +
                    "       ELSE\n" +
                    "       STR(ROUND(100.0 * ISNULL(TOTAL_2.SCAN_COUNT, 0) / CASE ISNULL(TOTAL_1.REQ_COUNT, 1) WHEN 0 THEN 1 ELSE ISNULL(TOTAL_1.REQ_COUNT, 1) END, 5), 10, 2) || '%'\n" +
                    "       END) FINISH_RATE,\n" +
                    "       CASE STR(TOTAL_3.IDENTICAL_COUNT, 10, 0) WHEN NULL THEN '0' ELSE STR(TOTAL_3.IDENTICAL_COUNT, 10, 0) END IDENTICAL_COUNT,\n" +
                    "       (CASE CHARINDEX('.', STR(ROUND(100.0 * ISNULL(TOTAL_3.IDENTICAL_COUNT, 0) / CASE ISNULL(TOTAL_2.SCAN_COUNT, 1) WHEN 0 THEN 1 ELSE ISNULL(TOTAL_2.SCAN_COUNT, 1) END, 5), 10, 2) || '%')\n" +
                    "       WHEN 1 THEN\n" +
                    "       '0' || STR(ROUND(100.0 * ISNULL(TOTAL_3.IDENTICAL_COUNT, 0) / CASE ISNULL(TOTAL_2.SCAN_COUNT, 1) WHEN 0 THEN 1 ELSE ISNULL(TOTAL_2.SCAN_COUNT, 1) END, 5), 10, 2) || '%'\n" +
                    "       ELSE\n" +
                    "       STR(ROUND(100.0 * ISNULL(TOTAL_3.IDENTICAL_COUNT, 0) / CASE ISNULL(TOTAL_2.SCAN_COUNT, 1) WHEN 0 THEN 1 ELSE ISNULL(TOTAL_2.SCAN_COUNT, 1) END, 5), 10, 2) || '%'\n" +
                    "       END) IDENTICAL_RATE,\n" +
                    "       TOTAL_1.TASK_STATUS \n" +
                    ...
                    "         WHERE EOCM.ORGANIZATION_ID = ASTA.ORGANIZATION_ID\n" +
                    "               AND ASTA.TASK_ID = AAST.TASK_ID\n" +
                    ...
                    "         GROUP BY AAST.TASK_ID) TOTAL_3\n" +
                    " WHERE TOTAL_1.TASK_ID *= TOTAL_2.TASK_ID\n" +
                    "       AND TOTAL_1.TASK_ID *= TOTAL_3.TASK_ID\n" +
                    "       AND ( ? = -1  OR TOTAL_1.ORGANIZATION_ID = ?)\n" +
                    "       AND ( " + SyBaseSQLUtil.isNull() + "  OR TOTAL_1.TASK_NO LIKE ?)\n" +
                    "       AND ( " + SyBaseSQLUtil.isNull() + "  OR TOTAL_1.TASK_NAME LIKE ?)\n"+
                    "       AND ( ? = '' OR TOTAL_1.CREATION_DATE >= ?)\n"+
                    "       AND ( ? = '' OR TOTAL_1.CREATION_DATE <= ?)" +
                    "		AND TOTAL_1.TASK_STATUS <> 'CANCELED' ";
