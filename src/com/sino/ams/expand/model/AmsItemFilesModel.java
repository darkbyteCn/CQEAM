...
public class AmsItemFilesModel extends BaseSQLProducer {
	...
	/**
	 * 功能：设备相关附件(AMS) AMS_ITEM_FILES 数据库SQL构造层构造函数
	...
	/**
	 * 功能：框架自动生成设备相关附件(AMS) AMS_ITEM_FILES数据插入SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成设备相关附件(AMS) AMS_ITEM_FILES数据更新SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成设备相关附件(AMS) AMS_ITEM_FILES数据删除SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成设备相关附件(AMS) AMS_ITEM_FILES数据详细信息查询SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成设备相关附件(AMS) AMS_ITEM_FILES多条数据信息查询SQLModel，请根据实际需要修改。
	...
	public SQLModel getDataMuxModel(){
		...
		String sqlStr = "SELECT "
			...
			+ " AMS_ITEM_FILES"
			+ " WHERE"
			+ "(LTRIM(?) IS NULL OR BARCODE LIKE '%' || ? || '%')"
			+ "(LTRIM(?) IS NULL OR FILE_DESC LIKE '%' || ? || '%')"
			+ "(LTRIM(?) IS NULL OR FILE_PATH LIKE '%' || ? || '%')"
			+ "(LTRIM(?) IS NULL OR SYSTEM_ID LIKE '%' || ? || '%')"
			+ "(LTRIM(CONVERT(VARCHAR, ?)) IS NULL OR CONVERT(VARCHAR, CREATION_DATE) LIKE '%' || CONVERT(VARCHAR, ?) || '%')"
			+ "(? IS NULL OR CREATED_BY = ?)"
			+ "(LTRIM(CONVERT(VARCHAR, ?)) IS NULL OR CONVERT(VARCHAR, LAST_UPDATE_DATE) LIKE '%' || CONVERT(VARCHAR, ?) || '%')"
			+ "(? IS NULL OR LAST_UPDATE_BY = ?)";
		sqlArgs.add(amsItemFiles.getBarcode());
		sqlArgs.add(amsItemFiles.getBarcode());
		sqlArgs.add(amsItemFiles.getFileDesc());
		sqlArgs.add(amsItemFiles.getFileDesc());
	...
	/**
	 * 功能：框架自动生成设备相关附件(AMS) AMS_ITEM_FILES页面翻页查询SQLModel，请根据实际需要修改。
	...
	public SQLModel getPageQueryModel(){
		...
		String sqlStr = "SELECT "
			...
			+ " WHERE"
			+ " (LTRIM(?) IS NULL OR BARCODE LIKE '%' || ? || '%')"
			+ "AND (LTRIM(?) IS NULL OR FILE_DESC LIKE '%' || ? || '%')"
			+ "AND (LTRIM(?) IS NULL OR FILE_PATH LIKE '%' || ? || '%')"
			+ "AND (LTRIM(?) IS NULL OR SYSTEM_ID LIKE '%' || ? || '%')"
			+ "AND (LTRIM(CONVERT(VARCHAR, ?)) IS NULL OR CONVERT(VARCHAR, CREATION_DATE) LIKE '%' || CONVERT(VARCHAR, ?) || '%')"
			+ "AND (? IS NULL OR CREATED_BY = ?)"
			+ "AND (LTRIM(CONVERT(VARCHAR, ?)) IS NULL OR CONVERT(VARCHAR, LAST_UPDATE_DATE) LIKE '%' || CONVERT(VARCHAR, ?) || '%')"
