...
public class RentAssetsRadioProducer {


	private SfUserDTO userAccount = null;

	public RentAssetsRadioProducer(SfUserDTO userAccount) {
	 ...
	 * 功能：构造资产归属范围单选按钮。
	 * @return String
	 * @throws com.sino.base.exception.HandlerException
     */
...
//			String[] valCates = AssetsWebAttributes.ASSETS_TREE_TYPES;
			String[] valCates = AssetsWebAttributes.RENT_ASSETS_TYPES;
//			String[] capCates = AssetsWebAttributes.CAPTIONS_TREE_TYPES;
			String[] capCates = AssetsWebAttributes.RENT_CAPTIONS_TYPES;
			...
			for (int i = 0; i < valCates.length; i++) {
				...
				if(assetsValue.equals(AssetsWebAttributes.RENT_ASSETS_TREE_DEPART)){//部门资产管理员判断
					if(!isDeptMgr){
						continue;
					}
				}
				if(assetsValue.equals(AssetsWebAttributes.RENT_ASSETS_TREE_COMPAN)){//公司资产管理员判断
					if(!isComMgr){
						continue;
					}
				}
				if(assetsValue.equals(AssetsWebAttributes.RENT_ASSETS_TREE_PROVIN)){//全省资产管理员判断
					if(!isProvMgr){
						continue;
					}
				}
