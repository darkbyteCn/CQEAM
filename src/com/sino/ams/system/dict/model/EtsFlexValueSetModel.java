...
public class EtsFlexValueSetModel extends BaseSQLProducer {
	 ...
	 */
	public EtsFlexValueSetModel(SfUserDTO userAccount, EtsFlexValueSetDTO dtoParameter) {
		super(userAccount, dtoParameter);
		sfUser = userAccount;
	}
	/**
	 * 功能：框架自动生成字典分类表(AMS) ETS_FLEX_VALUE_SET数据插入SQLModel，请根据实际需要修改。
	...
	public SQLModel getDataCreateModel(){
		...
		String sqlStr = "INSERT INTO "
			...
			+ " LAST_UPDATE_BY"
			+ ") VALUES ("
			+ " NEWID() ,  ?, ?, ?, ?, ?, ?, ? , CONVERT( VARCHAR , GETDATE() , 121 ) , CONVERT( VARCHAR , GETDATE() , 121 ) ,  ?)";
...
//		sqlArgs.add(etsFlexValueSet.getLastUpdateDate());
	...
	/**
	 * 功能：框架自动生成字典分类表(AMS) ETS_FLEX_VALUE_SET数据更新SQLModel，请根据实际需要修改。
	...
	public SQLModel getDataUpdateModel(){
		...
		List sqlArgs = new ArrayList();
		EtsFlexValueSetDTO etsFlexValueSet = (EtsFlexValueSetDTO)dtoParameter;
		String sqlStr = "UPDATE ETS_FLEX_VALUE_SET"
			+ " SET"
			+ " CODE = ?,"
			+ " NAME = ?,"
			+ " DESCRIPTION = ?,"
	...
	/**
	 * 功能：框架自动生成字典分类表(AMS) ETS_FLEX_VALUE_SET数据删除SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成字典分类表(AMS) ETS_FLEX_VALUE_SET数据详细信息查询SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成字典分类表(AMS) ETS_FLEX_VALUE_SET多条数据信息查询SQLModel，请根据实际需要修改。
	...
	/**
	 * 功能：框架自动生成字典分类表(AMS) ETS_FLEX_VALUE_SET页面翻页查询SQLModel，请根据实际需要修改。
	...
	public SQLModel getPageQueryModel(){
		...
		List sqlArgs = new ArrayList();
		EtsFlexValueSetDTO etsFlexValueSet = (EtsFlexValueSetDTO)dtoParameter;
//		String sqlStr = " SELECT * FROM ETS_FLEX_VALUE_SET EFVS ";
		String sqlStr = "SELECT "
//			+ " FLEX_VALUE_SET_ID , '' CODE , '' NAME , '' DESCRIPTION , '' CREATION_DATE , ENABLED, IS_INNER, '' MAINTAIN_FLAG  FROM ETS_FLEX_VALUE_SET  ";
			+ " FLEX_VALUE_SET_ID,"
			+ " CODE,"
			+ " NAME,"
			+ " DESCRIPTION,"
			+ " CREATION_DATE, "
			+ " (CASE ENABLED WHEN  'Y' THEN '是' ELSE '否' END) ENABLED,"
			+ " (CASE IS_INNER WHEN 'Y' THEN '是' ELSE '否' END) IS_INNER,"
			+ " (CASE MAINTAIN_FLAG WHEN 'Y' THEN '是' ELSE '否' END) MAINTAIN_FLAG"
			+ " FROM"
			+ " ETS_FLEX_VALUE_SET"
			+ " WHERE"
			+ " ( " + SyBaseSQLUtil.nullStringParam() + "  OR CODE LIKE ?)"
			+ " AND ( " + SyBaseSQLUtil.nullStringParam() + "  OR NAME LIKE ?)"
			+ " AND IS_INNER = ?"
			+ " AND MAINTAIN_FLAG = ?"
			+ " AND ENABLED = ?";
		SyBaseSQLUtil.nullStringParamArgs(sqlArgs, etsFlexValueSet.getCode() );
		SyBaseSQLUtil.nullStringParamArgs(sqlArgs, etsFlexValueSet.getName() );
