...
import com.sino.ams.system.dict.dao.EtsFlexAnalyseValueSetDAO;
import com.sino.ams.system.dict.dto.EtsFlexValueSetDTO;
import com.sino.ams.system.dict.model.EtsFlexAnalyseValueSetModel;
...
/**
 * <p>Title: EtsFlexAnalyseValueSetServlet</p>
 * <p>Description:程序自动生成服务程序“EtsFlexAnalyseValueSetServlet”，请根据需要自行修改</p>
...
public class EtsFlexAnalyseValueSetServlet extends BaseServlet {
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		if(req.getParameter("flexValueSetName") != null){
			String flexValueSetName = new String(req.getParameter("flexValueSetName").getBytes("ISO-8859-1"), "GBK");
		...
		try {
			...
			EtsFlexAnalyseValueSetDAO flexAnalyseValueSetDAO = new EtsFlexAnalyseValueSetDAO(user, dtoParameter, conn);
			if (action.equals("")) {
				dtoParameter.setEnabled(WebAttrConstant.TRUE_VALUE);
				dtoParameter.setIsInner(WebAttrConstant.FALSE_VALUE);
				dtoParameter.setMaintainFlag(WebAttrConstant.TRUE_VALUE);
				setRadioProp(req, dtoParameter);
				forwardURL = URLDefineList.DICT_ANALYSE_TYPE_QRY_PAGE;
			} else if (action.equals(WebActionConstant.QUERY_ACTION)) {
				setRadioProp(req, dtoParameter);
				BaseSQLProducer sqlProducer = new EtsFlexAnalyseValueSetModel(user, dtoParameter);
				...
				dtoParameter.setEnabled(WebAttrConstant.TRUE_VALUE);
				dtoParameter.setIsInner(WebAttrConstant.FALSE_VALUE);
				dtoParameter.setMaintainFlag(WebAttrConstant.FALSE_VALUE);
				setRadioProp(req, dtoParameter);
				...
				forwardURL = URLDefineList.DICT_ANALYSE_TYPE_DETAIL_PAGE;
			} else if (action.equals(WebActionConstant.DETAIL_ACTION)) {
				flexAnalyseValueSetDAO.setDTOClassName(EtsFlexValueSetDTO.class.getName());
				EtsFlexValueSetDTO etsFlexValueSet = (EtsFlexValueSetDTO)flexAnalyseValueSetDAO.getDataByPrimaryKey();
				if(etsFlexValueSet == null){
				...
				flexAnalyseValueSetDAO.setServletConfig(servletConfig);
				flexAnalyseValueSetDAO.createData();
				message = flexAnalyseValueSetDAO.getMessage();
				forwardURL = URLDefineList.DICT_ANALYSE_TYPE_QRY_SERVLET;
			} else if (action.equals(WebActionConstant.UPDATE_ACTION)) {
				ServletConfigDTO servletConfig = getServletConfig(req);
				flexAnalyseValueSetDAO.setServletConfig(servletConfig);
				flexAnalyseValueSetDAO.updateData();
				message = flexAnalyseValueSetDAO.getMessage();
				forwardURL = URLDefineList.DICT_ANALYSE_TYPE_QRY_SERVLET;
			} else {
