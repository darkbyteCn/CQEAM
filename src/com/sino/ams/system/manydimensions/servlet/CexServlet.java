...
import com.sino.ams.system.manydimensions.dao.CexDAO;
 ...
 * Function:		投资分类属性维护
 */
public class CexServlet extends BaseServlet {
    public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            CexDAO dao = new CexDAO(user, dto, conn);
			
			req.setAttribute(WebAttrConstant.AMS_ELEMENT_MATCH_DTO, dto);
            if (act.equals("")) {
				forwardURL = URLDefineList.CEX_PAGE;				
            } else if (act.equals(WebActionConstant.QUERY_ACTION)) {
            	BaseSQLProducer sqlProducer = new CexModel(user, dto);
    ...
    private void getAmsCexIds(HttpServletRequest req, AmsElementMatchDTO dto) throws UploadException {
        try {
            ...
            String[] exarr = reqParser.getParameterValues("amsCexId");
            if (exarr != null) {
                StringBuffer ids = new StringBuffer();
                for (int i = 0; i < exarr.length; i++) {
                    ids.append(exarr[i] + ",");
                }
                dto.setAmsCexId(ids.substring(0, ids.length()-1));
            }
