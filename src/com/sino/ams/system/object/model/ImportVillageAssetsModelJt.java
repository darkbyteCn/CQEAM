...
public class ImportVillageAssetsModelJt extends AMSSQLProducer {
    private SfUserDTO sfUser = null;

    public ImportVillageAssetsModelJt(SfUserDTO userAccount, ImportVillageAssetsDTOJt dtoParameter) {
...
//        ImportVillageAssetsDTOJt eoDTO = (ImportVillageAssetsDTOJt) dtoParameter;
        String sqlStr = "INSERT INTO AMS_VILLAGE_ASSETS_IMPORT_JT\n" +
                "  (COMPANY_CODE,\n" +
                "   BARCODE,\n" +
                "   ITEM_NAME,\n" +
                "   ITEM_SPEC,\n" +
...
//        sqlArgs.add(eoDTO.getProjectName());
...
//        sqlModel.setArgs(sqlArgs);
    ...
    public SQLModel validateSameBarcode(String barcode) throws SQLModelException {
        ...
        String sqlStr = "SELECT COUNT(1) BAR_QTY\n" +
                "FROM   AMS_VILLAGE_ASSETS_IMPORT_JT AVAI\n" +
                "WHERE  AVAI.BARCODE = ?\n" +
                "AND    AVAI.USER_ID = ?";
        sqlArgs.add(barcode);
        sqlArgs.add(userAccount.getUserId());
    ...
    public SQLModel hasErrorModel() throws SQLModelException {
        ...
        List sqlArgs = new ArrayList();
        ImportVillageAssetsDTOJt eoDTO = (ImportVillageAssetsDTOJt) dtoParameter;
        String sqlStr = "SELECT 1 FROM AMS_VILLAGE_ASSETS_IMPORT_JT AVAL WHERE AVAL.ERROR IS NOT NULL AND AVAL.USER_ID = ?";
        sqlArgs.add(userAccount.getUserId());
        sqlModel.setSqlStr(sqlStr);
        sqlModel.setArgs(sqlArgs);
        return sqlModel;
