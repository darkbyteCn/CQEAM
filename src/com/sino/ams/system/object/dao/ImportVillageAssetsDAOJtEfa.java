...
import com.sino.ams.system.object.model.ImportVillageAssetsModelJtEfa;
...
public class ImportVillageAssetsDAOJtEfa extends AMSBaseDAO {

    private final static String[] fieldNames = {              
            "companyCode", "barcode", "itemName", "itemSpec",
            "employeeNumber", "employeeName", "workorderObjectCode", "workorderObjectName", 
            "actualQty", "contentCode", "contentName", "price", "villageStartDate",
            "tfDepreciation", "tfNetAssetValue", "tfDeprnCost", "projectid", "projectName", 
            "depreciationAccount", "assetsCreateDate", "deprnLeftMonth", "dateRetired", "manufacturerName", 
            "scrapValue", "deprnAmount", "ytdDeprn", "deprnReserve", "impairAmount", "ytdImpairment", "impairReserve", 
            "constructStatus", "isShare", "remark"
    };

    public ImportVillageAssetsDAOJtEfa(SfUserDTO userAccount, AmsItemOnNetDTO dtoParameter, Connection conn) {
        super(userAccount, dtoParameter, conn);
    }

    protected void initSQLProducer(BaseUserDTO userAccount, DTO dtoParameter) {
        ImportVillageAssetsDTOJtEfa dtoPara = (ImportVillageAssetsDTOJtEfa) dtoParameter;
        super.sqlProducer = new ImportVillageAssetsModelJtEfa((SfUserDTO) userAccount, dtoPara);
    }

    public void deleteImportModel() throws SQLModelException, QueryException, DataHandleException {
        ImportVillageAssetsModelJtEfa eoModel = (ImportVillageAssetsModelJtEfa) sqlProducer;
    ...
    public boolean itemImportData(DTOSet dtoSet) throws DataHandleException {
        ...
        try {
            if (dtoSet != null && dtoSet.getSize() > 0) {
                ...
                ImportVillageAssetsModelJtEfa modelProducer = (ImportVillageAssetsModelJtEfa) sqlProducer;
...
//                    pstmt.setInt(fieldCount + 2, eoDTO.getAssetId());
                    pstmt.addBatch();
                    if (i % 100 == 0 && i > 0) {
                        pstmt.executeBatch();
                    }
    ...
    public String doVerifyData() throws DataHandleException {
        ...
        String errorMsg = "";
        try {
            String callStr = "{CALL dbo.CTZC_CHECK_DATA_EFA(?, ?)}";
    ...
    public String submitOrderDtl() throws DataHandleException {
        ...
        try {
            autoCommit = conn.getAutoCommit();
            conn.setAutoCommit(false);
            String callStr = "{CALL dbo.CTZC_TRANS_DATA_EFA(?, ?)}";
            cs = conn.prepareCall(callStr);
            cs.setInt(1, userAccount.getUserId());
            cs.registerOutParameter(2, Types.VARCHAR);
            cs.execute();
    ...
    public void doVerifyData(DTOSet dtoSet) throws SQLModelException, QueryException, ContainerException {
        if (dtoSet != null && dtoSet.getSize() > 0) {
            for (int i = 0; i < dtoSet.getSize(); i++) {
                ImportVillageAssetsDTOJtEfa villageDTO = (ImportVillageAssetsDTOJtEfa) dtoSet.getDTO(i);
...
//                }
    ...
    public void insertImprotErrorData(String barcode, String codeError) throws SQLModelException {
        try {
            ImportVillageAssetsModelJtEfa onNetModel = (ImportVillageAssetsModelJtEfa) sqlProducer;
    ...
    public DTOSet getImport() throws QueryException, SQLModelException {
        ...
        sq.setDTOClassName(ImportVillageAssetsDTOJtEfa.class.getName());
        sq.executeQuery();
        return sq.getDTOSet();
    }
