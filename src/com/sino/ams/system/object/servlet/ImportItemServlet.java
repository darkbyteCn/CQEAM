...
/**
 * Created by IntelliJ IDEA. User: su Date: 2009-4-26 Time: 10:27:05 Function:Excel实物资产成批调拨导入.
 */
			...
			for (int i = 0; i < dtoSet.getSize(); i++) {
				...
				try {
					...
					if (errmsg.indexOf("unique index") > -1) {
						throw new DataHandleException("第" + (i+1) + "条数据存在重复");
					} else {
						throw new DataHandleException("第" + (i+1) + "条数据存在错误:"
								+ ex.getMessage());
					}
				} catch (Exception ex) {
					String errmsg = ex.getMessage();
					operatorResult = false;
					if (errmsg.indexOf("unique index") > -1) {
						throw new DataHandleException("第" + i + "条数据存在重复");
					} else {
						throw new DataHandleException("第" + i + "条数据存在错误:"
								+ ex.getMessage());
					}
				}
			}
