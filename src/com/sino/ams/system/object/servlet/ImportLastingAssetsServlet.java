...
import com.sino.ams.system.object.dao.ImportLastingAssetsDAO;
 ...
 */
public class ImportLastingAssetsServlet extends BaseServlet {
	private static int startRowNum = 1;
	private static int columnNum = 21;
	...
	public void performTask(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
            ...
            ImportLastingAssetsDAO ImObDAO = new ImportLastingAssetsDAO(userAccount, null, conn);
...
//=========================================================
			...
			ReadLastingAssetsInfo xlsUtil = new ReadLastingAssetsInfo();
            ...
            ImportLastingAssetsModel onNetModel = new ImportLastingAssetsModel(userAccount, null);
			if (ImObDAO.importHasError()) {
                ...
                forwardURL = "/system/object/importLastingError.jsp";
            } else {
                DTOSet dtoSetTemp = ImObDAO.getImport();
				submitOrderDtl(dtoSetTemp, conn, userAccount);
                forwardURL = "/system/object/importLastingAssets.jsp";
				showMsg = "租赁资产导入成功！";
			}
            conn.commit();
	...
	private boolean itemImportData(DTOSet dtoSet, Connection conn, SfUserDTO userAccount) throws  DataHandleException, SQLModelException {
        ...
        if (dtoSet != null && dtoSet.getSize() > 0) {
			...
			ImportLastingAssetsModel modelProducer = new ImportLastingAssetsModel(userAccount, null);
			for (int i = 0; i < dtoSet.getSize(); i++) {
				LastingAssetsDTO eoDTO = (LastingAssetsDTO) dtoSet.getDTO(i);
			...
			if (dtoSet != null && dtoSet.getSize() > 0) {
                ...
                for (int i = 0; i < dtoSet.getSize(); i++) {
                    ...
                    if (!StrUtil.isEmpty(eoDTO.getWorkorderObjectCode())) {//获取ADDRESS_ID
                        int addressId = ImObDAO.getAddressId(eoDTO.getWorkorderObjectCode());
                        eoDTO.setAddressId(addressId);
                    }
                    if (!StrUtil.isEmpty(eoDTO.getEmployeeNumber())) {//获取EMPLOYEE_ID
                        int employeeId = ImObDAO.getEmployeeId(eoDTO.getEmployeeNumber());
    ...
    private int getNextRentId(Connection conn) throws SQLException {
		SeqProducer seqProducer = new SeqProducer(conn);
		return seqProducer.getStrNextSeq("AMS_RENT_INFO");
	}

    private int getNextSystemId(Connection conn) throws SQLException {
		SeqProducer seqProducer = new SeqProducer(conn);
		return seqProducer.getStrNextSeq("ETS_ITEM_INFO");
	}
}
