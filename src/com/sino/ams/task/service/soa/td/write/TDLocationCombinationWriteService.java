...
import com.sino.ams.task.dao.soa.TDLocationCombinationSearchDAO;
 ...
 * <p>描述: 通过SOA服务将EAM系统发生的地点变动同步到TD系统</p>
 ...
 * @version 1.0
 */
public class TDLocationCombinationWriteService extends AbstractTaskSOAService {

    /**
     * <p>功能说明：将EAM系统发生的地点变动同步到TD系统 </p>
     *
     * @throws com.sino.base.exception.DataHandleException
     *          将EAM系统发生的地点变动同步到TD系统出错时抛数据处理异常
     */
        ...
        try {
            ...
            if (rows != null && !rows.isEmpty()) {
                ...
                for (int i = 0; i < dataCount; i++) {
                    ...
                    TDSrvAssetLocCombDAO assetsUpdateDAO = new TDSrvAssetLocCombDAO(taskExecutor, null, conn);
    ...
    private String[] getOU2SynchronizeLocations(Connection conn, int organizationId) throws QueryException {
        ...
        try {
            TDLocationCombinationSearchDAO searchDAO = new TDLocationCombinationSearchDAO(taskExecutor, null, conn);
            RowSet rows = searchDAO.getTDOU2SynchronizeLocations(organizationId);
            if (rows != null && !rows.isEmpty()) {
                int dataCount = rows.getSize();
