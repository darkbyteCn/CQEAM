...
public class FlowTaskTool {
    ...
    public static boolean isExist(String tabName, String couName, int id, Connection conn) throws QueryException {
        SQLModel sqlModel = getSqlModel(tabName, couName, id);
        return hasNext(sqlModel, conn);
    }
	...
	private static SQLModel getSqlModel(String tabName, String couName,
		...
		String sqlStr = "SELECT " + " NULL TEMPFIELD" + " FROM " + tabName.toUpperCase()
    ...
    private static SQLModel getSqlModel(String tabName, List list) {
		SQLModel sqlModel = new SQLModel();
		String sqlStr = "SELECT " + " NULL TEMPNAME" + " FROM " + tabName.toUpperCase()
				+ " WHERE ";
		for (int i = 0; i < list.size(); i++) {
			SelectEmpty se = (SelectEmpty) list.get(i);
			if (se.isInt()) {
                if(StrUtil.isInteger(se.getLikeName())) {
                    int value = Integer.parseInt(se.getLikeName());
                    if(value > 0)
